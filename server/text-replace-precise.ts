...
/**
 * 计算 text_edit 叠字蒙版宽度的「字数缩放比例」。
 *
 * 只收窄、不放大：减字（新字数 < 原字数）时按比例收窄，下限 0.55 留余量防裁切；
 * 加字/等字时保持 1.0（原宽），加字所需空间仍由下游 createOgdEditMaskDataUrl
 * 的膨胀（boxDilateBinary radius=10）兜底，不在这里冒险扩洞。
 */
function computeTextEditWidthScale(oldLen: number, newLen: number): number {
...
/** 统计「可见字符数」（去掉空白）。中英混排按字符数近似，误差由 0.55 下限吸收。 */
function countVisibleChars(text: string): number {
  return (text || "").replace(/\s/g, "").length;
}

/**
 * 生成 text_edit「叠字」阶段的蒙版（透明 = 可写区）。
 *
 * 背景：叠字原本直接复用擦字阶段的膨胀蒙版（textEditDilatedMaskBuffer）。
 * 膨胀蒙版是为「加字留空间」调的，但字数变少时洞过宽 —— 模型为「填满」把字放大，
 * 甚至冲破画面（用户反馈「欢乐中国年」5 字被撑满 7 字宽的洞、左右出画）。
 * 这里按「新字数 / 原字数」把每个被改区域的洞在 x 方向收窄、保持原字高，
 * 让模型在正确宽度的洞里用原字号写字。
 *
 * ⚠️ 与擦字蒙版分离的原因：擦字必须用原宽（甚至膨胀）才能擦干净原字两端；
 * 叠字才需要收窄。两阶段共用一张蒙版时，收窄必然漏擦原字 —— 这正是
 * 「在前端 createSmartCopyEditMask 里直接收窄」会被否掉的根因。
 *
 * @returns 新蒙版 PNG；没有任何「改字」区域（全是删除/未改）时返回 null，
 *          调用方应回退到原蒙版。
 */
export async function buildTextEditOverlayMask(
  textRegions: DrawTextRegion[],
  editedText: string,
  width: number,
  height: number,
): Promise<Buffer | null> {
  const resolved = resolveRegionTargetTexts(textRegions, editedText);
  const editRegions = resolved.filter(
    item => item.changed && item.targetText && item.targetText.trim().length > 0,
  );
  if (editRegions.length === 0) return null;

  // 全不透明（alpha 255）：不透明 = 保留区，透明 = 可写区（与前端 createSmartCopyEditMask 同语义）
  const rgba = Buffer.alloc(width * height * 4, 255);
  const paddingX = Math.max(4, Math.round(width * 0.012));
  const paddingY = Math.max(4, Math.round(height * 0.012));

  for (const item of editRegions) {
    const region = item.region;
    const oldLen = countVisibleChars(region.text || "");
    const newLen = countVisibleChars(item.targetText || "");
    const scale = computeTextEditWidthScale(oldLen, newLen);
    const baseX0 = region.x * width;
    const baseW = region.width * width;
    const newW = baseW * scale;
    // 居中收窄：scale<1 时 shrink>0，洞在 x 方向向内收，保持与原文排版同一中心。
    const shrink = (baseW - newW) / 2;
    const x0 = Math.max(0, Math.floor(baseX0 + shrink - paddingX));
    const x1 = Math.min(width - 1, Math.ceil(baseX0 + shrink + newW + paddingX));
    const y0 = Math.max(0, Math.floor(region.y * height - paddingY));
    const y1 = Math.min(
      height - 1,
      Math.ceil((region.y + region.height) * height + paddingY),
    );
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
  ...
  return sharp(rgba, { raw: { width, height, channels: 4 }, limitInputPixels: false })
    .png()
    .toBuffer();
}
