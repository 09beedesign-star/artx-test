 ...
 *
 * 2026-09-13：美图条目随通道整体下线一并移除（账号已被上游停用）。
 */
...
function buildProviderHealth(): ProviderHealth[] {
  ...
  /**
   * 2026-09-13 移除了「MEITU / 美图开放平台」条目：账号被上游停用
   * （403，内层 `{"code":1003,"message":"access key is disabled"}`），通道整体下线。
   *
   * 📌 留一条给后来人的经验（别再重新踩）：美图那套凭据的键名是**裸的**
   * `ACCESS_KEY` / `SECRET_KEY`，**没有 `MEITU_` 前缀**。`MEITU_*` 那批变量
   * 全是网关地址、配方 ID、超时和蒙版参数，没有一个是凭据。所以当年按
   * `MEITU_API_KEY` 去探测会恒判「未配置」—— 接任何供应商前先去实现里确认
   * 它到底读哪个 env 键，不要照着供应商名字想当然加前缀。
   */
