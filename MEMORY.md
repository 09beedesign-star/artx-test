...
- Eraser AI rule: canvas eraser strokes call PicWish/佐糖 image object removal through `server/image-generation.ts#eraseWithPicWish`, using the documented `POST /api/tasks/visual/inpaint` and `GET /api/tasks/visual/inpaint/{task_id}` flow. It supports `sync=0` and `sync=1`, `image_file` or `image_url`, `mask_file` or `mask_url` or `rectangles`, 180s polling, and white-remove/black-protect masks matching the PicWish inpaint contract.
...
- PicWish response ID rule: successful eraser inpaint responses must include the PicWish task id as `providerTaskId` so browser-side verification and `docs/dev-log.md` can record it without relying only on Render logs.
