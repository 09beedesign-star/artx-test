...
- Eraser edge blending rule: after PicWish/佐糖 inpaint returns, `server/image-generation.ts#compositeEraseResultInsidePicWishMask` must feather the internal compositing mask edge before blending with the original image, so erased regions do not show hard seams; do not change outpaint, annotation, smart background, HD, layer editing, or watermark paths when tuning eraser blending.
