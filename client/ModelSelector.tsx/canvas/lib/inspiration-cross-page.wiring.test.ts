 ...
 *
 * 【用户的硬约束】
 * 「同一个灵感卡片的点赞数在两页都要一致，这是最终目标。」
 * （原话里还有「头像」，该功能已于 2026-09-18 按用户要求全站移除，
 *   但一致性约束对点赞/收藏计数依然成立。）
 *
 * 【为什么光有纯函数测试不够】
 * `inspiration-identity.ts` 和 `inspiration-metrics.ts` 都是纯函数，
 * 各自的单测早就全绿了 —— 但线上两页计数照样对不上。
 *
...
describe("跨页一致：两页必须消费同一个远程数据源", () => {
  ...
  it("首页兜底必须走共享出口，且两页都不许自己解析 CSV", () => {
     ...
     * 给 title 加个 trim / 前缀，降级时两页点赞数就重新对不上，
     * 而且不会有任何报错。
