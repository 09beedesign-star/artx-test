...
function withCanvasFrameAlpha(
  color: unknown,
  alpha = CANVAS_FRAME_BACKGROUND_ALPHA
) {
...
function SkillPointSelector({
  ...
  const updatePopoverPosition = useCallback(() => {
    ...
    const openAbove =
      spaceAbove >= Math.min(maxHeight, 260) || spaceAbove >= spaceBelow;
    ...
    const top = openAbove
      ...
      : Math.min(
          window.innerHeight - margin - availableHeight,
          rect.bottom + gap
        );
    setPopoverStyle({
      position: "fixed",
        ...
        createPortal(
          ...
          >
            ...
            {groupedSkills.map(([group, skills]) => (
              <div key={group}>
                <div
                  className="px-2.5 pb-1 pt-2 text-[11px] font-semibold"
                  style={{
                    color: isDark
...
function NodeWrapper({
  ...
  return (
    <div
      ...
      style={{
        ...style,
        background: (style?.background as string) || bg,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="text-white/75 type-caption flex-1 truncate">
            {title}
          </span>
          <button
        ...
        />
        <p className="text-white/51 type-caption mt-3">
          按 Esc 关闭 · 点击背景关闭
        </p>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <div style={{ flex: 1 }}>
          <div style={{ opacity: 0.55, marginBottom: 4, fontSize: 11 }}>
            行高
          </div>
          <input
            type="number"
            ...
            value={lineHeight}
            onChange={e =>
              onUpdate({ lineHeight: parseFloat(e.target.value) || 1.4 })
            }
            style={inputStyle}
          />
        </div>
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ opacity: 0.55, marginBottom: 6, fontSize: 11 }}>
          文字装饰
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <button
            style={toggleBtn(textDecoration === "underline")}
            onClick={() =>
              onUpdate({
                textDecoration:
          ...
          <button
            style={toggleBtn(textDecoration === "line-through")}
            onClick={() =>
              onUpdate({
                textDecoration:
          ...
          <button
            style={toggleBtn(textDecoration === "overline")}
            onClick={() =>
              onUpdate({
                textDecoration:
                  textDecoration === "overline" ? "none" : "overline",
              })
            }
          >
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[
            ...
            <button
              key={item.value}
              style={toggleBtn(textTransform === item.value)}
              onClick={() => onUpdate({ textTransform: item.value })}
            >
              {item.label}
            </button>
          ))}
    ...
    >
            ...
            <button
              ...
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.background =
                  textAlign === a ? activeBg : "transparent";
              }}
            >
              {alignIcons[a]}
            </button>
        ...
        >
          {FONT_FAMILIES.map(f => (
            <button
              ...
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.background =
                  fontFamily === f ? activeBg : "transparent";
              }}
            >
              {f}
            </button>
        ...
        >
          {FONT_WEIGHTS.map(w => (
            <button
              ...
              onClick={() => {
                onUpdate({ fontWeight: w.value });
                setShowWeightMenu(false);
              }}
              onMouseEnter={e => {
                if (fontWeight !== w.value)
                  (e.currentTarget as HTMLElement).style.background = hover;
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.background =
                  fontWeight === w.value ? activeBg : "transparent";
              }}
            >
              {w.label}
            </button>
      ...
      >
        ...
        <button
          ...
          onMouseLeave={e => {
            (e.currentTarget as HTMLElement).style.background = showAdvanced
              ? activeBg
              : "transparent";
          }}
        >
...
function AssetFloatingToolbar({
  ...
  const isFloatingToolDivider = (
    item: FloatingToolItem
  ): item is FloatingToolDivider => item.type === "divider";
    ...
    <div key={item.action} className="relative">
      ...
      <button
        ...
        style={{
          color: iconColor,
          background:
            item.action === "more" && moreOpen ? hoverBg : "transparent",
        }}
        onMouseEnter={e => {
  ...
  return (
    ...
    >
      ...
      >
        {tools.map(item =>
          isFloatingToolDivider(item)
            ? renderDivider(item.key)
            : renderButton(item)
        )}
      </div>
    </div>
...
function normalizeAssetAdjustments(value: unknown): AssetAdjustmentValues {
  const source =
    typeof value === "object" && value !== null
      ...
      : {};
  const read = (
    key: keyof AssetAdjustmentValues,
    fallback = DEFAULT_ASSET_ADJUSTMENTS[key]
  ) => {
  ...
  const legacyTemperature =
    typeof source.temperature === "number" ? source.temperature : 0;
  const legacyTint = typeof source.tint === "number" ? source.tint : 0;
  return {
    color: read(
      "color",
      Math.max(-100, Math.min(100, (legacyTemperature + legacyTint) / 2))
    ),
    brightness: read(
      "brightness",
      typeof source.exposure === "number" ? source.exposure : 0
    ),
...
function areAssetAdjustmentsEqual(
  a: AssetAdjustmentValues,
  b: AssetAdjustmentValues
) {
  return (
    a.color === b.color &&
...
function createAssetAdjustmentFilter(adjustments: AssetAdjustmentValues) {
  const brightness = Math.max(
    0.25,
    Math.min(2, 1 + adjustments.brightness / 130)
  );
  const contrast = Math.max(
    0.25,
    Math.min(2.2, 1 + adjustments.contrast / 150)
  );
  const saturation = Math.max(
    0,
    Math.min(2.6, 1 + adjustments.saturation / 120)
  );
  const hueRotate = adjustments.color * 1.8;
  const softness =
    adjustments.sharpness < 0
      ? ` blur(${Math.abs(adjustments.sharpness) * 0.025}px)`
      : "";
...
function sampleImageEdgeColor(image: HTMLImageElement) {
  ...
  try {
    ...
    for (let index = 0; index < maxSamples; index += 1) {
      const t = maxSamples <= 1 ? 0 : index / (maxSamples - 1);
      samples.push(readCanvasRgb(ctx, Math.round(t * (width - 1)), 0));
      samples.push(readCanvasRgb(ctx, Math.round(t * (width - 1)), height - 1));
      samples.push(readCanvasRgb(ctx, 0, Math.round(t * (height - 1))));
      samples.push(readCanvasRgb(ctx, width - 1, Math.round(t * (height - 1))));
    }
...
function getLoadedImageVisualFingerprint(image: HTMLImageElement) {
  try {
    const width = Math.max(1, image.naturalWidth || image.width);
    const height = Math.max(1, image.naturalHeight || image.height);
    const sampleSize = 8;
    ...
    for (let index = 0; index < pixels.length; index += 4) {
      grays.push(
        Math.round(
          (Number(pixels[index]) * 0.299 +
            Number(pixels[index + 1]) * 0.587 +
            Number(pixels[index + 2]) * 0.114) /
            16
        )
      );
    }
...
async function fetchCanvasImageBlob(src: string) {
  if (src.startsWith("data:")) return await (await fetch(src)).blob();
  const fetchDirect = () => fetch(src, { mode: "cors", credentials: "omit" });
  const fetchProxy = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      const isLocalFrontend =
        ...
        hostname === "127.0.0.1" ||
        hostname === "::1";
      const isViteOnlyLocal =
        isLocalFrontend && (port === "5173" || port === "5174");
      if (isLocalFrontend && !isViteOnlyLocal) {
...
function getImageProxyEndpointBase() {
  let baseUrl = getCanvasApiBaseUrl().replace(/\/+$/, "");
  if (typeof window !== "undefined") {
    const hostname = window.location.hostname;
    const port = window.location.port;
...
async function fetchImageProxyDataUrl(src: string) {
  ...
  try {
    const endpointBase = getImageProxyEndpointBase();
    const endpoint = `${endpointBase}/api/images/proxy?url=${encodeURIComponent(src)}`;
    ...
    return blob.size > 0 ? await blobToDataUrl(blob) : null;
  } catch {
    return null;
  }
}

function getImageProxyUrl(src: string) {
  if (!/^https?:\/\//i.test(src)) return src;
  const endpointBase = getImageProxyEndpointBase();
  return `${endpointBase}/api/images/proxy?url=${encodeURIComponent(src)}`;
}

async function loadExternalImageWithProxyFallback(src: string) {
  if (src.startsWith("data:")) {
    return { image: await loadImageForCanvas(src), localSrc: src };
  }
  try {
    const proxyDataUrl = await fetchImageProxyDataUrl(src);
    if (proxyDataUrl) {
      return {
        image: await loadImageForCanvas(proxyDataUrl),
        localSrc: proxyDataUrl,
      };
    }
  } catch {
...
async function loadReadableImageForCanvas(src: string) {
  if (src.startsWith("data:"))
    return { image: await loadImageForCanvas(src), revoke: () => {} };
  const blob = await fetchCanvasImageBlob(src);
...
async function applyAssetAdjustmentsToImage(
  src: string,
  adjustments: AssetAdjustmentValues
) {
  const readable = await loadReadableImageForCanvas(src);
  try {
    ...
    ctx.filter = createAssetAdjustmentFilter({
      ...adjustments,
      sharpness: Math.min(0, adjustments.sharpness),
    });
    ctx.drawImage(image, 0, 0, width, height);
    ctx.filter = "none";
...
async function createSocialMediaSizedImage(
  src: string,
  size: { width: number; height: number },
  crop: SocialMediaExportPayload["crop"]
) {
...
async function createMultiPlatformCoverBlob(
  ...
  const targetRatio = size.width / size.height;
  const baseScale =
    naturalW / naturalH > targetRatio
      ? size.width / naturalW
      : size.height / naturalH;
  const drawScale = baseScale * Math.max(0.2, transform.scale || 1);
  ...
  ctx.save();
  ctx.translate(
    size.width / 2 + transform.offsetX * size.width,
    size.height / 2 + transform.offsetY * size.height
  );
  ...
  const blob = await new Promise<Blob | null>(resolve =>
    canvas.toBlob(resolve, mimeType, format === "png" ? undefined : 0.94)
  );
...
function SocialMediaSizePanel({
  ...
  const defaultCoverTransform: CoverTransform = {
    offsetX: 0,
    offsetY: 0,
    scale: 1,
    rotation: 0,
  ...
  const [coverTransforms, setCoverTransforms] = useState<
    Record<string, CoverTransform>
  >({});
  const [activeCropPresetId, setActiveCropPresetId] = useState<string | null>(
    null
  );
  const dragRef = useRef<null | {
    startX: number;
    startY: number;
    startTransform: CoverTransform;
    bounds: DOMRect;
  }>(null);
  const [panelPosition, setPanelPosition] = useState(() => {
    if (typeof window === "undefined") return { left: 24, top: 24 };
    return {
      left: Math.max(16, window.innerWidth - 844),
      top: Math.max(16, Math.min(96, window.innerHeight - 620)),
    };
  });
  ...
  const selectedPresets = SOCIAL_MEDIA_SIZE_PRESETS.filter(item =>
    selectedPresetIds.includes(item.id)
  );
  ...
  const activeCropPreset =
    selectedPresets.find(item => item.id === activeCropPresetId) || null;
  const previewPreset =
    activeCropPreset ||
    selectedPresets[0] ||
    (validCustom
  ...
  const previewRatio = Math.max(
    0.2,
    Math.min(3.2, previewPreset.width / previewPreset.height)
  );
  const editablePresetId =
    activeCropPreset?.id ||
    selectedPresets[0]?.id ||
    (validCustom ? "custom" : null);
  const previewTransform =
    coverTransforms[previewPreset.id] || defaultCoverTransform;

  const togglePreset = (id: string) => {
    setSelectedPresetIds(prev => {
      if (!prev.includes(id)) return [...prev, id];
      const next = prev.filter(item => item !== id);
      if (activeCropPresetId === id) setActiveCropPresetId(null);
      return next;
    });
  };

  const toggleCropPreset = (id: string) => {
    setActiveCropPresetId(prev => (prev === id ? null : id));
  };
  ...
  const clearSelectedPresets = () => {
    ...
    setActiveCropPresetId(null);
  };

  const updatePresetTransform = (
    presetId: string,
    updater: (current: CoverTransform) => CoverTransform
  ) => {
    setCoverTransforms(prev => ({
      ...prev,
      [presetId]: updater(prev[presetId] || defaultCoverTransform),
    }));
  };

  const nudgePreview = (deltaX: number, deltaY: number) => {
    if (!editablePresetId) return;
    updatePresetTransform(editablePresetId, prev => ({
      ...prev,
      offsetX: Math.max(-1, Math.min(1, prev.offsetX + deltaX)),
      offsetY: Math.max(-1, Math.min(1, prev.offsetY + deltaY)),
    }));
  };

  const scalePreview = (delta: number) => {
    if (!editablePresetId) return;
    updatePresetTransform(editablePresetId, prev => ({
      ...prev,
      scale: Math.max(0.2, Math.min(4, Number((prev.scale + delta).toFixed(2)))),
    }));
  };
  ...
  const handlePanelPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!drag) return;
    const maxLeft =
      typeof window === "undefined" ? drag.startLeft : window.innerWidth - 160;
    const maxTop =
      typeof window === "undefined" ? drag.startTop : window.innerHeight - 120;
    setPanelPosition({
      left: Math.max(8, Math.min(maxLeft, drag.startLeft + event.clientX - drag.startX)),
      top: Math.max(8, Math.min(maxTop, drag.startTop + event.clientY - drag.startY)),
    });
  };
  ...
  const handleCropPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!activeCropPreset) return;
    updatePresetTransform(activeCropPreset.id, () => ({
      ...drag.startTransform,
  ...
  const handlePreviewWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    event.preventDefault();
    event.stopPropagation();
    scalePreview(event.deltaY > 0 ? -0.08 : 0.08);
  };

  const handleResetPreview = () => {
    if (editablePresetId) {
      updatePresetTransform(editablePresetId, () => defaultCoverTransform);
      return;
    }
    setCoverTransforms({});
  };
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <p style={{ color: text, fontSize: 14, fontWeight: 650 }}>
            多平台封面
          </p>
          <p style={{ color: sub, fontSize: 11, marginTop: 2 }}>
            可多选平台尺寸，单独裁切后一次性批量保存。
          </p>
        </div>
        <button
      ...
      <div className="min-h-0 flex flex-1">
        <div
          className="flex w-[360px] shrink-0 flex-col p-3"
        ...
        <div
          ...
          style={{
            ...
            background: field,
            border: `1px solid ${border}`,
            cursor: activeCropPreset ? "grab" : "default",
          }}
        ...
        >
          {imageSrc ? (
            <img
              ...
              style={{
                transform: `translate(${previewTransform.offsetX * 100}%, ${previewTransform.offsetY * 100}%) rotate(${previewTransform.rotation}deg) scale(${previewTransform.scale})`,
          ...
          <div
            ...
            style={{ boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.26)" }}
          />
          {activeCropPreset ? (
            <div
              className="absolute bottom-2 left-2 rounded-[var(--radius-md-design)] px-2 py-1.5"
              style={{
            ...
            >
              ...
              >
                {activeCropPreset.platform} · 拖拽移动，滚轮缩放
              </div>
              <div
                className="flex items-center gap-1"
                onPointerDown={event => event.stopPropagation()}
              >
                <button
                  type="button"
                  className="flex h-6 w-6 items-center justify-center rounded-[var(--radius-sm-design)]"
            ...
            <div
              className="absolute bottom-2 left-2 rounded-[var(--radius-md-design)] px-2 py-1"
              style={{
                background: "rgba(0,0,0,0.42)",
                color: "white",
                fontSize: 11,
        ...
        >
          <div className="mb-2 flex items-center justify-between gap-2">
            <label
              ...
              style={{ color: text, fontSize: 12, fontWeight: 650 }}
            >
              <input
                type="checkbox"
      ...
      <div
        className="flex shrink-0 gap-2 px-3 py-3"
        style={{ borderTop: `1px solid ${border}`, background: bg }}
      >
        <button
          className="h-9 flex-1 rounded-[var(--radius-md-design)] active:scale-95"
          style={{
            background: field,
            color: text,
            fontSize: 13,
        ...
        >
          {canGenerate
            ? `批量导出 ${selectedPresets.length + (validCustom ? 1 : 0)} 张`
            : "批量导出"}
        </button>
      </div>
    </div>
...
function AssetMoreCommandPanel({
  ...
  const [adjustments, setAdjustments] = useState<AssetAdjustmentValues>(
    initialAdjustments || DEFAULT_ASSET_ADJUSTMENTS
  );
  ...
  > = {
    mockup: {
      title: "多平台封面",
      description: "选择平台封面规格并导出本地图片。",
      actions: [],
    },
    expand: {
      title: "扩展",
      description: "按比例扩展画面边界，保留主体视觉。",
      actions: ["1:1", "4:5", "16:9"],
    },
    adjust: {
      title: "调整",
      description: "实时调整图片色彩、亮度、对比度、饱和度和锐利度。",
      actions: [],
    },
    crop: {
      title: "裁切",
      description: "选择裁切比例，图片节点会更新为新的尺寸。",
      actions: ["自由", "1:1", "3:4", "4:3", "16:9", "9:16"],
    },
    vector: {
      title: "矢量",
      description: "将图片轮廓转为可编辑矢量元素。",
      actions: ["提取轮廓", "扁平化", "高清矢量"],
    },
  };
      ...
      <div className="p-3">
        ...
        <button
          ...
          onClick={() =>
            onApply(
              current.title,
              command === "adjust" ? adjustments : undefined
            )
          }
        >
          {command === "adjust"
    ...
    >
      ...
      >
        ...
        >
          <span
            className="type-caption"
            style={{ color: accent, fontWeight: 700, lineHeight: 1 }}
          >
            {count}
          </span>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 p-3">
        <div
          className="w-9 h-9 rounded-[var(--radius-md-design)] flex items-center justify-center flex-shrink-0"
          style={{
            background: chipBg,
        ...
        <div
          className="w-9 h-9 rounded-[var(--radius-md-design)] flex items-center justify-center flex-shrink-0 cursor-pointer"
        ...
        <textarea
          ...
          onKeyDown={e => {
            if (e.key === "Enter" && !e.shiftKey) {
              e.preventDefault();
              if (prompt.trim()) {
                toast("AI 正在处理图片", { description: prompt.slice(0, 50) });
                setPrompt("");
              }
            }
          }}
      ...
      >
        ...
        <button
          className="flex items-center gap-1 px-2 py-1 rounded-[var(--radius-md-design)] type-caption transition-opacity hover:opacity-80"
        ...
        <span className="type-caption" style={{ color: chipText }}>
          1×
        </span>
        <button
          className="w-7 h-7 rounded-[var(--radius-pill)] flex items-center justify-center transition-all hover:opacity-90"
    ...
    >
      ...
      >
        ...
        {ann.editing ? (
            ...
            <textarea
              ...
              onKeyDown={e => {
                ...
                if (e.key === "Enter" && (e.ctrlKey || e.metaKey))
                  onUpdate(ann.id, { text: draft, editing: false });
              }}
            />
            <div
              style={{
          ...
          >
            {ann.text || (
              <span style={{ color: subColor, fontStyle: "italic" }}>
                点击编辑注释...
              </span>
            )}
          </div>
...
function AssetInlineNote({
  ...
  const removeNote = () =>
    onUpdate({ note: "", noteOpen: false, noteEditing: false });
...
function AssetNodeComponent({
  ...
  const localSrc = (data as Record<string, unknown>).localSrc as
    | string
    | undefined;
  const asset = GENERATED_ASSETS.find(a => a.id === (data.assetId as string));
  const isGeneratingImage = Boolean(
    (data as { isGeneratingImage?: boolean }).isGeneratingImage
  );
  const isGenerationFailed = Boolean(
    (data as { isGenerationFailed?: boolean }).isGenerationFailed
  );
  const isRemovingBackground = Boolean(
    (data as { isRemovingBackground?: boolean }).isRemovingBackground
  );
  const isErasingImage = Boolean(
    (data as { isErasingImage?: boolean }).isErasingImage
  );
  const isExtractingText = Boolean(
    (data as { isExtractingText?: boolean }).isExtractingText
  );
  const isAiProcessingImage =
    isGeneratingImage ||
    isGenerationFailed ||
  ...
  const processingLines = (() => {
    ...
    return [
      processingLabel.slice(0, midpoint),
      processingLabel.slice(midpoint),
    ].filter(Boolean);
  })();
  ...
  const [eraseBrushSize, setEraseBrushSize] = useState<number>(
    Number((data as { eraseBrushSize?: number }).eraseBrushSize) || 42
  );
  ...
  const cropX = Math.max(
    0,
    Math.min(100, Number((data as { cropX?: number }).cropX ?? 0))
  );
  const cropY = Math.max(
    0,
    Math.min(100, Number((data as { cropY?: number }).cropY ?? 0))
  );
  const cropW = Math.max(
    1,
    Math.min(100 - cropX, Number((data as { cropW?: number }).cropW ?? 100))
  );
  const cropH = Math.max(
    1,
    Math.min(100 - cropY, Number((data as { cropH?: number }).cropH ?? 100))
  );
	  const frameClipInsets = (
	    data as {
	      frameClipInsets?: {
	        top: number;
	        right: number;
	      ...
	      frameClipActive?: boolean;
	    }
	  ).frameClipInsets;
	  const hasFrameClipInsets =
	    Boolean((data as { frameClipActive?: boolean }).frameClipActive) &&
	    Boolean(frameClipInsets);
	  const frameClipStyle: React.CSSProperties | undefined = hasFrameClipInsets
	    ? {
  ...
  // 初始尺寸：以自然尺寸比例计算
  const naturalWidth = localSrc
    ? 720
    : Math.max(1, asset?.width || (data.imgW as number) || 260);
  const naturalHeight = localSrc
    ? 960
    : Math.max(1, asset?.height || (data.imgH as number) || 200);
  const maxNodeSide = 360;
  const minNodeSide = 120;
  const initScale = Math.min(
    1,
    maxNodeSide / Math.max(naturalWidth, naturalHeight)
  );
      ...
      setFlowNodes(nds =>
        nds.map(n =>
          n.id === nodeId
            ? {
                ...n,
                style: { ...n.style, width: initW, height: initH },
                data: {
  ...
  useEffect(() => {
    ...
    if (typeof nextW === "number" && nextW > 0 && nextW !== imgW)
      setImgW(nextW);
    if (typeof nextH === "number" && nextH > 0 && nextH !== imgH)
      setImgH(nextH);
  ...
  const handleResizeMouseDown = useCallback(
    (e: React.MouseEvent, direction: "nw" | "ne" | "se" | "sw" = "se") => {
      ...
      window.dispatchEvent(
        new CustomEvent("asset-resize-active", { detail: { active: true } })
      );
      setIsResizing(true);
      // 等比缩放：使用当前显示尺寸计算宽高比
      ...
      const getNextResize = (clientX: number, clientY: number) => {
        ...
        const nextX = drag.direction.includes("w")
          ? drag.startNodeX + drag.startW - newW
          : drag.startNodeX;
        const nextY = drag.direction.includes("n")
          ? drag.startNodeY + drag.startH - newH
          : drag.startNodeY;
      ...
      const onUp = (mu: MouseEvent) => {
        ...
        window.dispatchEvent(
          new CustomEvent("asset-resize-active", { detail: { active: false } })
        );
        setIsResizing(false);
        setImgW(newW);
        setImgH(newH);
        ...
        window.dispatchEvent(
          new CustomEvent("asset-resize-end", {
            detail: {
              nodeId,
              newW,
              newH,
              nextX,
              nextY,
              startNodeX: drag.startNodeX,
              startNodeY: drag.startNodeY,
              startW: drag.startW,
              startH: drag.startH,
            },
          })
  ...
  const handleNodeCtxMenu = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const handleImageAnnotateClick = useCallback(
  ...
  const handleCropEdgeMouseDown = useCallback(
    (e: React.MouseEvent, edge: "left" | "right" | "top" | "bottom") => {
      e.preventDefault();
      e.stopPropagation();
      cropDragRef.current = {
        edge,
      ...
      const onMove = (event: MouseEvent) => {
        ...
        if (!drag) return;
        const dx =
          ((event.clientX - drag.startClientX) / Math.max(1, dispW)) * 100;
        const dy =
          ((event.clientY - drag.startClientY) / Math.max(1, dispH)) * 100;
        const minSize = 12;
        setCropRect(() => {
          ...
          if (isExpanding) {
            if (drag.edge === "left") {
              ...
              next.w = Math.max(
                100 - newX,
                drag.startRect.w + drag.startRect.x - newX
              );
              next.x = newX;
            }
            if (drag.edge === "right") {
              next.w = Math.max(
                100 - drag.startRect.x,
                Math.min(260, drag.startRect.w + dx)
              );
            }
            if (drag.edge === "top") {
              ...
              next.h = Math.max(
                100 - newY,
                drag.startRect.h + drag.startRect.y - newY
              );
              next.y = newY;
            }
            if (drag.edge === "bottom") {
              next.h = Math.max(
                100 - drag.startRect.y,
                Math.min(260, drag.startRect.h + dy)
              );
            }
            return next;
          }
          if (drag.edge === "left") {
            const newX = Math.max(
              0,
              Math.min(
                drag.startRect.x + drag.startRect.w - minSize,
                drag.startRect.x + dx
              )
            );
          ...
          if (drag.edge === "right") {
            next.w = Math.max(
              minSize,
              Math.min(100 - drag.startRect.x, drag.startRect.w + dx)
            );
          }
          if (drag.edge === "top") {
            const newY = Math.max(
              0,
              Math.min(
                drag.startRect.y + drag.startRect.h - minSize,
                drag.startRect.y + dy
              )
            );
          ...
          if (drag.edge === "bottom") {
            next.h = Math.max(
              minSize,
              Math.min(100 - drag.startRect.y, drag.startRect.h + dy)
            );
          }
          return next;
        });
  ...
  const confirmCrop = useCallback(() => {
    window.dispatchEvent(
      new CustomEvent("asset-crop-commit", {
        detail: {
          nodeId,
          cropRect,
          startW: dispW,
          startH: dispH,
          sourceSrc: displaySrc,
        },
      })
    );
  ...
  const cancelCrop = useCallback(() => {
    window.dispatchEvent(
      new CustomEvent("asset-crop-cancel", { detail: { nodeId } })
    );
  }, [nodeId]);

  const applyExpandRatio = useCallback(
    (ratio: number | null) => {
      if (!ratio) {
  ...
  const cancelExpand = useCallback(() => {
    window.dispatchEvent(
      new CustomEvent("asset-expand-cancel", { detail: { nodeId } })
    );
  }, [nodeId]);
  ...
  const cancelErase = useCallback(() => {
    window.dispatchEvent(
      new CustomEvent("asset-erase-cancel", { detail: { nodeId } })
    );
  }, [nodeId]);

  const closeExtractedTextPanel = useCallback(
  ...
  const updateInlineNote = useCallback(
        ...
        detail: {
          ...
          panelScreenRect: extractedTextPanelRef.current
            ? {
                ...
                right:
                  extractedTextPanelRef.current.getBoundingClientRect().right,
                bottom:
                  extractedTextPanelRef.current.getBoundingClientRect().bottom,
              }
            : undefined,
        },
      })
      ...
      >
        ...
        >
          ...
          {isAiProcessingImage ? (
            <div
              className={
                isGeneratingImage && !isGenerationFailed
                  ? "artx-ai-generation-loading absolute inset-0 flex flex-col items-center justify-center gap-3"
              ...
              >
                {[
                  ...
                  <button
                    key={item.label}
                    type="button"
                    className="type-caption rounded-[var(--radius-md-design)] px-2.5 py-1 hover:opacity-80"
                    style={{
                      color: "white",
                    ...
                    onClick={() => applyCropRatio(item.ratio)}
                  >
                    {item.label}
                  </button>
                ))}
          ...
          {isExpanding && !isAiProcessingImage && (
            <div
              className="absolute inset-0 nodrag nopan"
              style={{ zIndex: 92, overflow: "visible" }}
            >
              <div
                className="absolute"
                style={{
              ...
              >
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => applyExpandRatio(item.ratio)}
                  >
                    {item.label}
                  </button>
                ))}
          ...
          {isErasing && !isAiProcessingImage && (
            <div
              className="absolute inset-0 nodrag nopan"
              style={{ zIndex: 95, overflow: "visible" }}
            >
              <canvas
                ref={eraseCanvasRef}
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  ...
                  <input
                    ...
                    aria-label="橡皮尺寸"
                    onChange={event =>
                      setEraseBrushSize(Number(event.target.value))
                    }
            ...
            !isExtractingText && (
              <button
                ...
                onClick={event => {
                  ...
                  event.stopPropagation();
                  window.dispatchEvent(
                    new CustomEvent<ImageRegenerateRequestDetail>(
                      "asset-regenerate-request",
                      { detail: regenerateDetail }
                    )
                  );
                }}
              >
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: "2px solid white",
                    boxShadow:
                      "0 0 0 1px rgba(0,0,0,0.20), 0 2px 6px rgba(0,0,0,0.45)",
                    zIndex: 80,
                  }}
...
function ChatNodeComponent({
  ...
  const aiBg = isDark
    ? "oklch(0.58 0.22 290 / 0.15)"
    : "oklch(0.58 0.22 290 / 0.08)";
  const aiBorder = isDark
    ? "oklch(0.58 0.22 290 / 0.25)"
    : "oklch(0.58 0.22 290 / 0.2)";
  ...
  const messages = [
    ...
    },
    {
      role: "ai",
      content:
        "已为你的跑鞋品牌设计了一套视觉资产，包含英雄图、产品特写和运动员穿着图，突出性能与材质。",
    },
  ];
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="w-5 h-5 rounded-[var(--radius-md-design)] flex items-center justify-center"
          style={{ background: "oklch(0.58 0.22 290 / 0.2)" }}
        >
          <MessageSquare size={11} style={{ color: "oklch(0.72 0.22 290)" }} />
        </div>
        <span className="type-caption" style={{ color: text }}>
          AI 对话
        </span>
      </div>
      <div
        className="flex flex-col gap-2 p-3 overflow-y-auto nodrag nopan"
        style={{ maxHeight: 260 }}
      >
        {messages.map((msg, i) => (
          <div
          ...
          >
            <div
              className="rounded-[var(--radius-md-design)] px-3 py-2 type-caption leading-relaxed max-w-[85%] whitespace-pre-line"
              style={{
      ...
      <div className="px-3 pb-3 nodrag nopan">
        <div
          className="flex items-center gap-2 rounded-[var(--radius-md-design)] px-3 py-2"
          style={{ background: inputBg, border: `1px solid ${inputBorder}` }}
        >
          <input
            className="flex-1 bg-transparent type-caption outline-none"
            style={{ color: text }}
            placeholder="继续对话..."
            onClick={e => e.stopPropagation()}
          />
...
function PromptNodeComponent({
  ...
  return (
    ...
    >
      ...
      <div className="px-3 pb-3 nodrag nopan">
        <button
          className="w-full py-1.5 rounded-[var(--radius-md-design)] type-caption flex items-center justify-center gap-1.5 transition-opacity hover:opacity-90 disabled:opacity-60"
          style={{ background: "oklch(0.58 0.22 290)", color: "white" }}
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast("Prompt 节点已生成", {
                description: result.text.slice(0, 90),
              });
            } catch (error) {
              const message =
...
function TextNodeComponent({
  ...
  const hoverBorder =
    !selected && isHovered ? "rgba(160,160,180,0.55)" : "transparent";
  const hoverShadow =
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        >
          {textContent || (
            <span style={{ opacity: 0.35 }}>双击编辑文字...</span>
          )}
        </div>
      )}
      <Handle
...
function CanvasFrameNode({
  ...
  const handleResizeMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMouseUp = (mu: MouseEvent) => {
        ...
        window.dispatchEvent(
          new CustomEvent("canvas-frame-resize", {
  ...
  const commitTitleEdit = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("canvas-frame-title-change", {
  ...
  const borderColor = selected
    ...
    : isDark
      ...
      : "oklch(0 0 0 / 18%)";
  // 使用用户选择的背景色，默认深灰色
  const bg = withCanvasFrameAlpha(
    data.bgColor || (data.originalBgColor as string) || "#2a2a30"
  );
  const labelColor = isDark ? "oklch(0.69 0.010 270)" : "oklch(0.66 0.010 270)";
  const handleColor = isDark
    ? "oklch(0.65 0.22 290 / 0.80)"
    : "oklch(0.50 0.20 290 / 0.80)";
  const handleNodeCtxMenu = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
...
function ShapeNodeComponent({
      ...
      const applyPointerPosition = (clientX: number, clientY: number) => {
        ...
        setAnchors(prev => {
          ...
          if (draggingAnchorRef.current)
            draggingAnchorRef.current.historyPushed = true;
          window.dispatchEvent(
            new CustomEvent("shape-anchor-offset", {
  ...
  const handleDoubleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      setFlowNodes(nds =>
        nds.map(n =>
          n.id === id
            ? { ...n, data: { ...n.data, anchorEditMode: true, anchors } }
            : n
        )
      );
    },
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent) => {
      ...
      e.stopPropagation();
      window.dispatchEvent(
        new CustomEvent("shape-ctx-open", {
  ...
  const buildPath = (pts: { x: number; y: number }[]) => {
    ...
    if (shapeType === "line")
      return `M ${pts[0]?.x ?? 0} ${pts[0]?.y ?? h / 2} L ${pts[1]?.x ?? w} ${pts[1]?.y ?? h / 2}`;
    if (shapeType === "arrow") {
      const x1 = pts[0]?.x ?? 0;
      const y1 = pts[0]?.y ?? h / 2;
      const x2 = pts[1]?.x ?? w;
      const y2 = pts[1]?.y ?? h / 2;
    ...
    return (
      pts.map((a, i) => `${i === 0 ? "M" : "L"} ${a.x} ${a.y}`).join(" ") + " Z"
    );
  };
...
function PenNodeComponent({
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent, type: "anchor" | "in" | "out", idx: number) => {
      e.preventDefault();
      e.stopPropagation();
          ...
          prev.map((a, i) => {
            ...
            if (dt === "anchor") {
              ...
              if (a.type === "smooth") {
                // 对称手柄
                const len = Math.sqrt(
                  newOutDx * newOutDx + newOutDy * newOutDy
                );
                ...
                return {
                  ...
                  outDx: newOutDx,
                  outDy: newOutDy,
                  inDx: len > 0 ? (-newOutDx / len) * inLen : a.inDx,
                  inDy: len > 0 ? (-newOutDy / len) * inLen : a.inDy,
                };
              }
              ...
              if (a.type === "smooth") {
                ...
                return {
                  ...
                  inDx: newInDx,
                  inDy: newInDy,
                  outDx: len > 0 ? (-newInDx / len) * outLen : a.outDx,
                  outDy: len > 0 ? (-newInDy / len) * outLen : a.outDy,
                };
              }
      ...
      const onUp = () => {
        ...
        setFlowNodes(nds =>
          nds.map(n =>
            n.id === id ? { ...n, data: { ...n.data, anchors: anchors } } : n
          )
        );
        window.removeEventListener("mousemove", onMove);
        window.removeEventListener("mouseup", onUp);
  ...
  const toggleClosed = useCallback(() => {
    setFlowNodes(nds =>
      nds.map(n =>
        n.id === id ? { ...n, data: { ...n.data, closed: !isClosed } } : n
      )
    );
              ...
              <g key={i}>
                {showIn && (a.inDx !== 0 || a.inDy !== 0) && (
                  <line
                    ...
                    strokeWidth={1 / vpZoom}
                    strokeDasharray={`${3 / vpZoom},${2 / vpZoom}`}
                  />
                )}
        ...
        anchors.map((a, i) => (
          <div
            ...
            style={{
              ...
              background:
                selectedAnchorIdx === i ? "oklch(0.65 0.22 290)" : "white",
              border: `${borderFlowPx}px solid oklch(0.65 0.22 290)`,
              cursor: "move",
              zIndex: 20,
            ...
            onContextMenu={e => {
              e.preventDefault();
              e.stopPropagation();
              setCtxMenu({ x: e.clientX, y: e.clientY, anchorIdx: i });
            }}
          />
        ))}
      ...
      {ctxMenu && (
        ...
        >
          {[
            ...
            {
              label: "删除锚点",
              action: () => deleteAnchor(ctxMenu.anchorIdx),
            },
          ].map(item => (
            <button
              key={item.label}
...
function nextCanvasTopZ(nodes: Node[]) {
  return (
    Math.max(
      0,
      ...nodes.map(node => (typeof node.zIndex === "number" ? node.zIndex : 0))
    ) + 1
  );
}
...
function buildSkillAppliedImagePrompt(input: {
  ...
  return [
    ...
    imagePrompt && imagePrompt !== input.userPrompt.trim()
      ? `路由优化后的图像提示：${imagePrompt}`
      : "",
  ]
    .filter(Boolean)
    .join("\n\n");
...
function getSkillPreferredRatio(
  activeSkill: PendingSkillLoad | null,
  fallback = "1:1"
) {
...
function getCanvasNodeSize(node: Node): CanvasNodeSize {
  ...
  const nodeWithRuntimeSize = node as Node & {
  ...
  const measuredWidth =
    styleWidth ??
    nodeWithRuntimeSize.measured?.width ??
    nodeWithRuntimeSize.width;
  const measuredHeight =
    styleHeight ??
    nodeWithRuntimeSize.measured?.height ??
    nodeWithRuntimeSize.height;
  if (
...
async function getVisibleAssetImageSourceFromNode(node: Node) {
  if (node.type !== "asset") return "";
  const data = node.data as Record<string, unknown>;
  const baseSrc = getAssetNodeImageSource(node);
  if (!baseSrc) return "";
  const hasPreview = Object.prototype.hasOwnProperty.call(
    data,
    "assetAdjustmentPreview"
  );
  const adjustments = normalizeAssetAdjustments(
    hasPreview ? data.assetAdjustmentPreview : data.assetAdjustments
  );
  if (
    !hasPreview &&
    areAssetAdjustmentsEqual(adjustments, DEFAULT_ASSET_ADJUSTMENTS)
  ) {
    return baseSrc;
  }
  if (areAssetAdjustmentsEqual(adjustments, DEFAULT_ASSET_ADJUSTMENTS)) {
    return baseSrc;
  }
  return applyAssetAdjustmentsToImage(baseSrc, adjustments);
}
...
function createInitialSocialArtboardNode(
  project?: WorkspaceHistoryProject | null
): Node[] {
...
const TEST_CANVAS_STATE_RESET_KEY =
  "artx:canvas-state:reset-test-canvases-20260620";
const CANVAS_IMAGE_GENERATION_TASKS_STORAGE_KEY =
  "artx:canvas-image-generation-tasks";
...
type PersistedImageGenerationTask = Omit<
  ImageGeneratorPayload,
  "generationId" | "status" | "images"
> & {
  generationId: string;
  status: "pending" | "completed" | "failed";
  updatedAt: number;
...
function getImageGenerationStartedAt(input: {
  ...
  const backgroundStartedAt =
    typeof input.backgroundStartedAt === "number"
      ? input.backgroundStartedAt
      : undefined;
  const updatedAt =
...
function ensureTestCanvasStateReset() {
  ...
  const shouldRemoveCanvasKey = (key: string) => {
    if (
      !key.startsWith(CANVAS_STATE_STORAGE_PREFIX) &&
      !key.startsWith(CANVAS_STATE_SESSION_PREFIX)
    )
      return false;
    const projectId = key
  ...
  for (let index = window.sessionStorage.length - 1; index >= 0; index -= 1) {
    ...
    if (key && shouldRemoveCanvasKey(key))
...
function imageGenerationTaskImageKey(
  projectId: string,
  generationId: string,
  index: number
) {
...
function writePersistedImageGenerationTasks(
  tasks: PersistedImageGenerationTask[]
) {
  if (typeof window === "undefined") return;
  const pruned = tasks
    .slice()
  ...
  try {
    ...
    const lightweight = pruned.map(task =>
      task.status === "completed"
        ? {
            ...task,
            images: undefined,
            consumedAt: task.consumedAt || Date.now(),
          }
        : task
    );
    try {
...
function persistImageGenerationTask(
  detail: ImageGeneratorPayload,
  ...
  const lightweightImages = detail.images?.map((image, index) => ({
    ...
    storedKey: image.src?.startsWith("data:")
      ? imageGenerationTaskImageKey(projectId, generationId, index)
      : undefined,
  }));
...
function markImageGenerationTaskBackgroundStarted(
  projectId: string,
  generationId: string
) {
  const tasks = readPersistedImageGenerationTasks();
  ...
  const next = tasks.map(task => {
    ...
    return {
      ...task,
      backgroundStartedAt: task.backgroundStartedAt || Date.now(),
      updatedAt: Date.now(),
    };
  });
...
function dispatchImageGenerationTask(
  detail: ImageGeneratorPayload,
  ...
  if (nextDetail.status === "completed" && nextDetail.images?.length) {
    void persistImageGenerationTaskImages(
      nextDetail.projectId,
      nextDetail.generationId,
      nextDetail.images
  ...
  window.dispatchEvent(
    new CustomEvent("image-generator-submit", { detail: nextDetail })
  );
}
...
function getRegenerableImageNodeDetail(
  ...
  data: Record<string, unknown>,
  displaySize: { w: number; h: number }
): ImageRegenerateRequestDetail | null {
  if (data.assetType !== "AI 生成") return null;
  const prompt =
    typeof data.generationPrompt === "string" ? data.generationPrompt : "";
  const model =
    typeof data.generationModel === "string" ? data.generationModel : "";
  const ratio =
    typeof data.generationRatio === "string" ? data.generationRatio : "";
  const style =
    typeof data.generationStyle === "string" ? data.generationStyle : "";
  ...
  return {
    ...
    displaySize,
    sourceBackgroundSrc:
      typeof data.generationSourceBackgroundSrc === "string"
        ? data.generationSourceBackgroundSrc
        : typeof data.sourceBackgroundSrc === "string"
          ? data.sourceBackgroundSrc
          : undefined,
    imageSrc:
...
function markImageGenerationTaskConsumed(
  projectId: string,
  generationId: string
) {
  const tasks = readPersistedImageGenerationTasks();
...
async function consumeCompletedImageGenerationTasks(projectId: string) {
  ...
  writePersistedImageGenerationTasks(
    tasks.map(task =>
      matching.some(
        item =>
          item.generationId === task.generationId &&
          (item.projectId || "p1") === (task.projectId || "p1")
      )
  ...
  const imageEntries = images
    .map((image, index) =>
      image.src?.startsWith("data:")
        ? {
            key: imageGenerationTaskImageKey(projectId, generationId, index),
            localSrc: image.src,
          }
        : null
    )
...
async function hydrateImageGenerationTaskImages(
  projectId: string,
  task: PersistedImageGenerationTask
) {
  const images = task.images || [];
  const missing = images.filter(image => image.storedKey && !image.src);
  if (missing.length === 0) {
    return images.filter(
      (image): image is { src: string; width: number; height: number } =>
        Boolean(image.src)
    );
  }
  const db = await openCanvasImageDb();
  ...
  await new Promise<void>(resolve => {
    ...
    missing.forEach(image => {
      ...
      request.onsuccess = () => {
        if (typeof request.result === "string" && image.storedKey)
  ...
  return images
    .map(image => ({
      src:
        image.src ||
        (image.storedKey ? restored.get(image.storedKey) : undefined) ||
        "",
      width: image.width,
      height: image.height,
    }))
...
async function persistCanvasNodeImagePayloads(
  ...
  const imageEntries = removePendingImageGenerationNodes(nodes)
    ...
    .map(node => {
      const data = node.data as Record<string, unknown>;
      const localSrc = typeof data.localSrc === "string" ? data.localSrc : "";
      return localSrc.startsWith("data:")
        ? { key: canvasImagePayloadKey(projectId, node.id), localSrc }
        : null;
    })
    .filter(Boolean) as { key: string; localSrc: string }[];
  if (imageEntries.length === 0) return;
...
function normalizeCanvasFrameNode(node: Node): Node {
  ...
  const rawBg =
    ...
    data.bgColor ||
    (node.style as Record<string, unknown> | undefined)?.background ||
    "#2a2a30";
...
function getCanvasStateCoverSource(nodes: Node[]) {
  const imageNode = nodes.find(
    node =>
      node.type === "asset" &&
      typeof (node.data as Record<string, unknown>)?.localSrc === "string"
  );
  return imageNode
    ? ((imageNode.data as Record<string, unknown>).localSrc as string)
    : null;
}
...
async function createCanvasCoverThumbnail(src: string) {
  ...
  const scale = Math.min(
    1,
    maxSide /
      Math.max(image.naturalWidth || maxSide, image.naturalHeight || maxSide)
  );
  const width = Math.max(
    1,
    Math.round((image.naturalWidth || maxSide) * scale)
  );
  const height = Math.max(
    1,
    Math.round((image.naturalHeight || maxSide) * scale)
  );
  const canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = height;
...
function getImportedImageDisplaySize(
  naturalWidth: number,
...
function normalizeDroppedImageUrl(value: string, baseUrl?: string) {
  const source = value
    .trim()
    .replace(/^url\((['"]?)(.*?)\1\)$/i, "$2")
    .replace(/\\\//g, "/")
...
function extractUrlsFromText(value: string) {
  const decoded = value
    ...
    .replace(/&quot;/g, "\"")
    .replace(/&#39;/g, "'");
  const textCandidates = [decoded];
  try {
    const uriDecoded = decodeURIComponent(decoded);
    if (uriDecoded !== decoded) textCandidates.push(uriDecoded);
  } catch {
    // Some drag payloads contain partial percent-encoding; keep the original text.
  }
  Array.from(decoded.matchAll(/(?:url|u|src|image|img|media|poster|thumb|cover)=([^&\s"'<>]+)/gi)).forEach(match => {
    const raw = match[1] || "";
    try {
      textCandidates.push(decodeURIComponent(raw));
    } catch {
  ...
  return Array.from(
    new Set(
      textCandidates.flatMap(text => [
        ...
        ...Array.from(text.matchAll(/\/\/[^\s"'<>\\]+/gi)).map(match => match[0] || ""),
        ...Array.from(text.matchAll(/data:image\/[a-z0-9.+-]+;base64,[a-z0-9+/=]+/gi)).map(match => match[0] || ""),
      ])
    )
  );
}
...
function getDataTransferStringItems(dataTransfer: DataTransfer | null | undefined) {
  return Array.from(dataTransfer?.items || []).filter(
    item => item.kind === "string"
  );
}

function readDataTransferItemString(item: DataTransferItem) {
  return new Promise<{ type: string; value: string }>(resolve => {
    try {
      item.getAsString(value => resolve({ type: item.type, value: value || "" }));
    } catch {
...
function getDataTransferTypeEntries(dataTransfer: DataTransfer | null | undefined) {
  return Array.from(dataTransfer?.types || [])
    .map(type => {
      try {
        return { type, value: dataTransfer?.getData(type) || "" };
      } catch {
        return { type, value: "" };
      }
    })
...
function fileLooksLikeImage(file: File) {
  return (
    ...
    /\.(?:png|jpe?g|gif|webp|avif|svg|bmp|heic|heif|tiff?)$/i.test(file.name)
  );
}

function getInlineComposerTextWidth(value: string) {
  if (!value) return 0;
  const fallbackWidth = Array.from(value).reduce(
    (total, char) => total + (/[\u4e00-\u9fff\uff00-\uffef]/.test(char) ? 13 : 7),
    0
  );
  if (typeof document === "undefined") return Math.ceil(fallbackWidth + 14);
  const canvas = document.createElement("canvas");
  const context = canvas.getContext("2d");
  if (!context) return Math.ceil(fallbackWidth + 14);
  context.font =
    "400 12px Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  return Math.ceil(context.measureText(value).width + 14);
}

function extractImageSourcesFromDataTransfer(
  dataTransfer: DataTransfer | null | undefined,
  extraEntries: Array<{ type: string; value: string }> = []
) {
  const sources: string[] = [];
  const addSource = (value: string, baseUrl?: string) => {
    const normalized = normalizeDroppedImageUrl(value, baseUrl);
  ...
  const htmlEntries = [
    html,
    ...extraEntries
      .filter(entry => /html/i.test(entry.type))
      .map(entry => entry.value),
  ].filter(Boolean);
  htmlEntries.forEach(htmlValue => {
    ...
    const baseUrl =
      ...
      dataTransfer?.getData("text/plain")?.split(/\r?\n/).find(line =>
        /^https?:\/\//i.test(line.trim())
      ) ||
      extraEntries.find(entry =>
        /uri|url|plain|text/i.test(entry.type) &&
        /^https?:\/\//i.test(entry.value.trim())
      )?.value.trim().split(/\r?\n/).find(line =>
        /^https?:\/\//i.test(line.trim())
      ) ||
      undefined;
    extractUrlsFromText(htmlValue).forEach(value => addSource(value, baseUrl));
    Array.from(doc.querySelectorAll("img")).forEach(img => {
      [
        img.getAttribute("src") || img.src,
        img.getAttribute("currentSrc") || "",
        getFirstSrcsetCandidate(img.getAttribute("srcset")),
        getFirstSrcsetCandidate(img.getAttribute("data-srcset")),
        ...
        img.getAttribute("data-backup") || "",
        img.getAttribute("data-orig-file") || "",
        img.getAttribute("data-media") || "",
        img.getAttribute("data-pin-media") || "",
        img.getAttribute("data-hi-res-src") || "",
      ].forEach(value => addSource(value, baseUrl));
    });
    Array.from(doc.querySelectorAll("source")).forEach(source => {
      addSource(source.getAttribute("src") || "", baseUrl);
      addSource(source.getAttribute("data-src") || "", baseUrl);
      addSource(getFirstSrcsetCandidate(source.getAttribute("srcset")), baseUrl);
      addSource(getFirstSrcsetCandidate(source.getAttribute("data-srcset")), baseUrl);
      addSource(getFirstSrcsetCandidate(source.getAttribute("data-lazy-srcset")), baseUrl);
    });
    Array.from(doc.querySelectorAll("video[poster], [data-poster]")).forEach(element => {
      addSource(element.getAttribute("poster") || "", baseUrl);
      addSource(element.getAttribute("data-poster") || "", baseUrl);
    });
    Array.from(
      doc.querySelectorAll(
        'meta[property="og:image"], meta[property="og:image:url"], meta[name="twitter:image"], meta[name="twitter:image:src"], link[rel="image_src"]'
      )
    ).forEach(element => {
      addSource(element.getAttribute("content") || element.getAttribute("href") || "", baseUrl);
    });
    Array.from(doc.querySelectorAll("a[href]")).forEach(anchor => {
      const href = anchor.getAttribute("href") || "";
      if (/(\.(?:png|jpe?g|gif|webp|avif|svg|bmp|heic|heif|tiff?)(?:[?#].*)?$|\/pins\/\d+|\/pin\/\d+|\/photo\/\d+|\/photos\/\d+)/i.test(href)) {
        addSource(href, baseUrl);
      }
    });
    Array.from(doc.querySelectorAll("[style]")).forEach(element => {
      const style = element.getAttribute("style") || "";
      const matches = Array.from(style.matchAll(/url\((['"]?)(.*?)\1\)/g));
      matches.forEach(match => addSource(match[2] || "", baseUrl));
    });
    const pageUrl = typeof baseUrl === "string" ? baseUrl.trim() : "";
    if (pageUrl && (sources.length === 0 || sources.every(source => source.startsWith("blob:")))) {
      addSource(pageUrl);
    }
  });
  ...
  mozUrl
    .split(/\r?\n/)
    .map(line => line.trim())
    .forEach(line => addSource(line));

  addSource(dataTransfer?.getData("text/x-moz-url-data") || "");
  addSource(dataTransfer?.getData("public.url") || "");
  addSource(dataTransfer?.getData("public.file-url") || "");
  addSource(dataTransfer?.getData("application/x-moz-file-promise-url") || "");
  ...
  plain
    ...
    .map(line => line.trim())
    .forEach(line => addSource(line));
  extractUrlsFromText(plain).forEach(line => addSource(line));

  extraEntries.forEach(entry => {
    const value = entry.value.trim();
    if (!value) return;
    if (/json|uri|url|plain|text|html|image|download/i.test(entry.type)) {
      extractUrlsFromText(value).forEach(url => addSource(url));
      value
        .split(/\r?\n/)
        .map(line => line.trim())
        .forEach(line => addSource(line));
...
async function extractImageSourcesFromDataTransferAsync(
  dataTransfer: DataTransfer | null | undefined
) {
  const typeEntries = getDataTransferTypeEntries(dataTransfer);
  const extraEntries = await Promise.all(
    getDataTransferStringItems(dataTransfer).map(readDataTransferItemString)
  );
  return extractImageSourcesFromDataTransfer(dataTransfer, [
    ...typeEntries,
    ...extraEntries,
  ]);
}

function extractImageSourcesFromClipboardText(
  htmlValues: string[],
  plainValues: string[]
) {
  return extractImageSourcesFromDataTransfer(null, [
    ...htmlValues
      .filter(Boolean)
      .map(value => ({ type: "text/html", value })),
    ...plainValues
      .filter(Boolean)
      .map(value => ({ type: "text/plain", value })),
  ]);
}

function clipboardPayloadHasImageContent(
  dataTransfer: DataTransfer | null | undefined
) {
  ...
  const html = dataTransfer?.getData("text/html") || "";
  if (html && /<(img|source|video)\b|data:image\/|https?:\/\/[^\s"'<>]+?\.(?:png|jpe?g|gif|webp|avif|svg)/i.test(html))
    return true;
...
function dataTransferMayContainExternalDrop(
  dataTransfer: DataTransfer | null | undefined
) {
  const types = Array.from(dataTransfer?.types || []);
  if (types.includes("application/x-artx-composer-token")) return false;
...
function BottomPromptBar({
  ...
  const resizePromptTextarea = useCallback(
    (input: HTMLTextAreaElement | null) => {
      ...
      const nextHeight = Math.min(
        ...
        Math.max(promptTextareaMinHeight, input.scrollHeight)
      );
      input.style.height = `${nextHeight}px`;
      input.style.overflowY =
        input.scrollHeight > promptTextareaMaxHeight ? "auto" : "hidden";
    },
    []
  );
  ...
  const handleSend = async (overridePrompt?: string) => {
    const effectivePrompt =
      typeof overridePrompt === "string" ? overridePrompt : prompt.trim();
      ...
      try {
        if (activeSkill) {
          ...
          const finalImagePrompt = buildSkillAppliedImagePrompt({
            ...
            skillContext,
            userPrompt:
              visiblePrompt || `请生成一张${activeSkill.subcategory}图片。`,
        ...
        if (decision.mode === "image") {
          ...
          const payload: ImageGeneratorPayload = {
            ...
            generationId,
            displaySize:
              targetDisplaySize || getImageDisplaySizeForRatio(ratio),
          };
          dispatchImageGenerationTask(
            { ...payload, status: "pending" },
            projectId
        ...
        const textResult = await callLLM({
          ...
          prompt:
            submittedPrompt || visiblePrompt || "请基于当前上下文回复用户。",
        });
        toast("AI 已回复", {
          description: (
            decision.reply ||
  ...
  useEffect(() => {
    ...
    try {
      ...
      autoRunModelRef.current = ALL_AI_MODEL_OPTIONS.some(
  ...
  const showReferencePreview = useCallback(
    (
      ...
      asset: ImageGeneratorReferenceAsset
    ) => {
      if (!asset.src) return;
  ...
  const handleSkillChange = useCallback(
    (skill: PendingSkillLoad | null) => {
      ...
      if (!skill) return;
      setPrompt(prev =>
        prev.trim() ? prev : `帮我生成一张${skill.subcategory}：`
      );
      window.setTimeout(() => {
        resizePromptTextarea(textareaRef.current);
        textareaRef.current?.focus();
      ...
      >
        ...
        >
          <textarea
            ...
            onPaste={event => {
              if (!clipboardPayloadHasImageContent(event.clipboardData)) return;
              event.preventDefault();
              toast("图片不能粘贴到提示词输入框", {
                description: "请粘贴到画布，或选中画布图片后作为引用素材使用",
              });
            }}
            rows={rows}
            className="composer-scrollbar-hidden w-full whitespace-pre-wrap break-words bg-transparent type-caption leading-relaxed resize-none outline-none"
            style={{
              color: text,
        ...
        <div
          className="flex min-w-0 items-center gap-2 px-3 pb-3"
          style={{ paddingTop: 8 }}
        >
          <ModelSelector model={model} onChange={setModel} isDark={isDark} />
          <div
          ...
          <button
            ...
            style={{
              color: isDark ? "oklch(0.69 0.010 270)" : "oklch(0.69 0.010 270)",
            }}
          ...
          <span
            className="type-caption"
            style={{
              color: isDark ? "oklch(0.38 0.008 270)" : "oklch(0.62 0.008 270)",
            }}
          >
            {isSending
              ? "AI 处理中"
          ...
          >
            {isSending ? (
              <RefreshCw size={13} color="white" className="animate-spin" />
            ) : (
              <Send
                size={13}
                color={
...
function NodeContextMenu({
  ...
  const items = menu.pasteOnly
    ? pasteItems
    : isGroupContainerMenu
      ? groupContainerItems
      : isSelectionMenu
        ? selectionItems
        : singleItems;
    ...
    >
      {items.map((item, i) =>
        item === null ? (
          <div
            key={`div-${i}`}
            style={{ height: 1, background: divider, margin: "2px 0" }}
          />
        ) : (
          <button
            key={item.action}
          ...
          >
            <span
              style={{
                color: item.color,
                opacity: item.action === "delete" ? 1 : 0.75,
                flexShrink: 0,
              }}
            >
              {item.icon}
...
function GroupContainerOverlay({
  ...
  const containerBg = isDark
    ? "oklch(0.58 0.22 290 / 0.07)"
    : "oklch(0.58 0.22 290 / 0.05)";
  const containerBorder = isDark
    ? "oklch(0.65 0.20 290 / 0.45)"
    : "oklch(0.55 0.20 290 / 0.35)";
      ...
      {isEntering && (
        <div
          ...
          style={{
            ...
            fontSize: 10,
            color: isDark
              ? "oklch(0.72 0.22 50 / 0.75)"
              : "oklch(0.55 0.18 50 / 0.80)",
            fontWeight: 500,
            pointerEvents: "none",
            userSelect: "none",
          }}
...
function LassoEraser({
  isDark,
  onCut,
}: {
  isDark: boolean;
  onCut: (rect: LassoRect) => void;
}) {
  ...
  return (
    ...
    >
      {/* Hint */}
      <div
        className="absolute top-3 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-[var(--radius-pill)] type-caption pointer-events-none"
        style={{
          background: isDark ? "rgba(255,80,80,0.15)" : "rgba(200,50,50,0.1)",
          border: `1px solid ${borderColor}`,
          color: isDark ? "rgba(255,150,150,0.9)" : "rgba(180,40,40,0.9)",
        }}
      >
...
function ZoomControlBar({
  ...
  const btnClass =
    "w-8 h-8 flex items-center justify-center rounded-[var(--radius-md-design)] transition-all active:scale-90 hover:opacity-80";
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            boxShadow: locked
              ? "0 0 0 1px oklch(0.68 0.29 25 / 0.35), 0 0 18px oklch(0.62 0.28 25 / 0.26)"
              : "none",
          }}
          ...
          onMouseEnter={e =>
            (e.currentTarget.style.background = locked
              ? "oklch(0.62 0.28 25 / 0.24)"
              : hoverBg)
          }
          onMouseLeave={e =>
            (e.currentTarget.style.background = locked
              ? "oklch(0.62 0.28 25 / 0.18)"
              : "transparent")
          }
        >
          {locked ? (
...
function BackButton({ isDark }: { isDark: boolean }) {
  ...
  return (
    <div
      ref={backButtonRef}
      className="absolute"
      style={{ top: 12, left: 12, zIndex: 101 }}
    >
      <button
        onClick={e => {
          e.stopPropagation();
        ...
        style={{
          background: open
            ? isDark
              ...
              : "oklch(0.50 0.18 280)"
            : isDark
              ? "oklch(0.18 0.02 270 / 0.95)"
              : "oklch(0.97 0.004 270 / 0.95)",
  ...
  const requestCanvasReferences = useCallback(
    () =>
      new Promise<ImageGeneratorReferenceAsset[]>(resolve => {
        window.dispatchEvent(
          new CustomEvent("image-generator-reference-request", {
            detail: {
              resolve: (assets: ImageGeneratorReferenceAsset[]) =>
                resolve(assets),
            },
          })
        );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          className="flex h-7 w-7 items-center justify-center rounded-[var(--radius-md-design)] hover:opacity-75"
          style={{ color: sub }}
          onClick={onClose}
          aria-label="关闭图像生成器"
        >
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {ratios.map(item => (
                <button
                  key={item}
                  type="button"
                  style={controlButtonStyle(ratio === item)}
...
const FONT_DESIGN_PURPOSES: FontDesignPurpose[] = [
  "Logo 字",
  "海报标题",
  "品牌标题",
  "社媒封面",
...
const FONT_DESIGN_STYLES: FontDesignStyle[] = [
  "高级极简",
  "潮流酸性",
  "国潮书法",
  "科技未来",
...
const FONT_DESIGN_STYLE_PREVIEWS: Record<
  FontDesignStyle,
  { image: string; sample: string; description: string }
> = {
  高级极简: {
    image: new URL(
      "../../../../inspiration-images/prompt-02.jpg",
      import.meta.url
    ).href,
    sample: "MONO",
  ...
  潮流酸性: {
    image: new URL(
      "../../../../inspiration-images/prompt-43.jpg",
      import.meta.url
    ).href,
    sample: "ACID",
  ...
  国潮书法: {
    image: new URL(
      "../../../../inspiration-images/prompt-11.jpg",
      import.meta.url
    ).href,
    sample: "山海",
  ...
  科技未来: {
    image: new URL(
      "../../../../inspiration-images/prompt-24.jpg",
      import.meta.url
    ).href,
    sample: "FUTURE",
  ...
  可爱软萌: {
    image: new URL(
      "../../../../inspiration-images/prompt-48.jpg",
      import.meta.url
    ).href,
    sample: "POP",
  ...
  奢华杂志: {
    image: new URL(
      "../../../../inspiration-images/prompt-13.jpg",
      import.meta.url
    ).href,
    sample: "VOGUE",
  ...
  街头涂鸦: {
    image: new URL(
      "../../../../inspiration-images/prompt-45.jpg",
      import.meta.url
    ).href,
    sample: "STREET",
  ...
  二次元标题: {
    image: new URL(
      "../../../../inspiration-images/prompt-33.jpg",
      import.meta.url
    ).href,
    sample: "ANIME",
  ...
  复古港风: {
    image: new URL(
      "../../../../inspiration-images/prompt-31.jpg",
      import.meta.url
    ).href,
    sample: "RETRO",
  ...
  欧美海报: {
    image: new URL(
      "../../../../inspiration-images/prompt-10.jpg",
      import.meta.url
    ).href,
    sample: "POSTER",
...
function buildFontDesignPrompt(input: {
  ...
  return [
    ...
    input.extraPrompt.trim()
      ? `Additional user direction: ${input.extraPrompt.trim()}`
      : "",
    "Critical text rules:",
...
function FontDesignDialog({
  ...
  const hasUnsavedInput =
    textValue.trim().length > 0 || extraPrompt.trim().length > 0;
  ...
  const optionButtonStyle = (active: boolean): React.CSSProperties => ({
    ...
    fontSize: 12,
    transition:
      "background 0.16s ease, border-color 0.16s ease, transform 0.16s ease, opacity 0.16s ease",
  });
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <span
              className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-lg-design)]"
              style={{
                color: aiAccent,
                background: "oklch(0.64 0.22 285 / 0.14)",
              }}
            >
            ...
            <div>
              <p
                className="type-caption"
                style={{ color: text, fontSize: 14, fontWeight: 750 }}
              >
                字体设计
              </p>
              <p
        ...
        <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto overflow-x-hidden p-4 lg:grid-cols-[315px_minmax(0,1fr)]">
          <div className="min-w-0 space-y-3">
            <div
              className="overflow-hidden rounded-[var(--radius-lg-design)]"
            ...
            >
              <div className="relative h-28 overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  ...
                  >
                    {stylePreset} · {activeStyle.description}
                  </div>
                </div>
              </div>
              <div className="p-3">
                <div className="mb-2 flex items-center justify-between">
                  <p className="type-caption" style={{ color: sub }}>
                    设计文字
                  </p>
                  <span className="type-caption" style={{ color: sub }}>
                    {detectFontDesignLanguage(textValue || "文字")} ·{" "}
                    {textValue.trim().length}/80
                  </span>
                </div>
                <textarea
                  value={textValue}
            ...
            <div
              ...
              style={{ background: panelBg, border: `1px solid ${border}` }}
            >
              <p className="mb-2 type-caption" style={{ color: sub }}>
                用途
              </p>
              <div className="grid grid-cols-2 gap-2">
                {FONT_DESIGN_PURPOSES.map(item => (
                  <button
                    ...
                    type="button"
                    className="active:scale-95"
                    style={optionButtonStyle(purpose === item)}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {FONT_DESIGN_RATIOS.map(item => (
                    <button
                      ...
                      type="button"
                      className="active:scale-95"
                      style={optionButtonStyle(ratio === item)}
                      onClick={() => setRatio(item)}
                    >
                      {item}
                    </button>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {FONT_DESIGN_COUNTS.map(item => (
                    <button
                      ...
                      type="button"
                      className="active:scale-95"
                      style={optionButtonStyle(count === item)}
                      onClick={() => setCount(item)}
                    >
                      {item}
                    </button>
          ...
          <div className="min-w-0 space-y-3">
            ...
            >
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <span
                  ...
                  style={{
                    color: aiAccent,
                    background: "oklch(0.64 0.22 285 / 0.12)",
                    ...
                    >
                      <span className="relative block h-20 overflow-hidden">
                        ...
                        <span className="absolute bottom-2 left-2 right-2 flex items-center justify-between gap-2">
                          <span
                            className="min-w-0 type-caption"
                            style={{
                              color: "white",
                              fontWeight: 850,
                              letterSpacing: 0,
            ...
            <div
              className="flex items-center justify-between rounded-[var(--radius-lg-design)] px-3 py-2.5"
              style={{ background: panelBg, border: `1px solid ${border}` }}
            >
              <div>
                <p
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              color: canGenerate ? "#000" : sub,
              boxShadow: canGenerate
                ? "0 14px 34px rgba(197,237,71,0.30)"
                : "none",
            ...
            onMouseEnter={event => {
              ...
              event.currentTarget.style.boxShadow =
                "0 18px 42px rgba(197,237,71,0.38)";
            }}
            onMouseLeave={event => {
              event.currentTarget.style.background = canGenerate
                ? accent
                : hoverBg;
              event.currentTarget.style.boxShadow = canGenerate
                ? "0 14px 34px rgba(197,237,71,0.30)"
                : "none";
            }}
            onClick={handleGenerate}
          >
            {isGenerating ? (
              ...
              <FontDesignIcon size={15} cutoutBg={accent} />
            )}
            <span className="type-caption">
              {isGenerating ? "生成中" : "生成字体设计"}
            </span>
          </button>
        </div>
      </div>
...
function CanvasTopToolPalette({
  ...
  const handleToolClick = (id: string) => {
    if (id === "shape") {
      ...
      setActive(id);
      window.dispatchEvent(
        new CustomEvent("tool-mode-change", { detail: { mode: id } })
      );
      return;
    }
    if (id === "draw") {
      ...
      // 广播当前铅笔参数
      window.dispatchEvent(
        new CustomEvent("draw-params", {
          detail: { color: drawColor, width: drawWidth },
        })
      );
      return;
    }
    ...
    if (id === "pen") {
      toast("钢笔工具", {
        description:
          "单击添加锚点 · 拖拽创建手柄 · Enter/Esc 完成 · Alt+单击切换直角 · 双击关闭路径",
      });
    } else {
      toast(tools.find(t => t.id === id)?.label ?? "", {
          ...
          >
            <input
              ...
              onChange={e => {
                ...
                window.dispatchEvent(
                  new CustomEvent("draw-params", {
                    detail: { color: e.target.value, width: drawWidth },
                  })
                );
              }}
              style={{
          ...
          >
            ...
            <input
              ...
              onChange={e => {
                ...
                window.dispatchEvent(
                  new CustomEvent("draw-params", {
                    detail: { color: drawColor, width: v },
                  })
                );
              }}
              style={{
            ...
            <span
              style={{
                fontSize: 12,
                color: isDark ? "rgba(255,255,255,0.69)" : "rgba(0,0,0,0.35)",
                userSelect: "none",
              }}
            >
              Px
...
// ── Canvas Search Bar ───────────────────────────────────────────
function SaveProjectConfirmDialog({
  isDark,
  project,
  onCancel,
  onSave,
...
const CANVAS_ASSISTANT_TEXT_MODEL_STORAGE_KEY =
  "artx:canvas-assistant-text-model";
const CANVAS_ASSISTANT_MODEL_TAB_STORAGE_KEY =
  "artx:canvas-assistant-model-tab";
const CANVAS_ASSISTANT_AUTO_MODE_STORAGE_KEY =
  "artx:canvas-assistant-auto-mode";
const CANVAS_ASSISTANT_AUTO_DEFAULT_VERSION_KEY =
...
function getCanvasRenderableImageSrc(src: string) {
  ...
  if (!uploadPath.startsWith("/uploads/")) return trimmed;
  let baseUrl = getCanvasApiBaseUrl().replace(/\/+$/, "");
  if (typeof window !== "undefined") {
    const hostname = window.location.hostname;
    const isLocalFrontend =
...
function deserializeCanvasAssistantMessages(
  raw: string | null
  ...
  try {
    ...
    return parsed
      ...
      .map(item => {
        const backup =
          item.imageBackup && typeof item.imageBackup.src === "string"
            ? {
                ...item.imageBackup,
                src: getCanvasRenderableImageSrc(item.imageBackup.src),
              }
            : item.imageBackup;
        return {
          ...item,
...
function createAssistantTextSegment(text = ""): AssistantComposerSegment {
  return {
    id: `seg-text-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    type: "text",
    text,
  };
}
...
function createAssistantImageSegment(
  ...
  return {
    id: `seg-image-${asset.id}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    type: "image",
    asset,
  };
}

function createAssistantAnnotationSegment(
  annotation: AnnotationReference
): AssistantComposerSegment {
  return {
    id: `seg-annotation-${annotation.id}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    type: "annotation",
    annotation,
  };
}

function normalizeAssistantComposerSegments(
  segments: AssistantComposerSegment[]
) {
...
function getAssistantComposerImages(segments: AssistantComposerSegment[]) {
  ...
  return segments
    .filter(
      (
        segment
      ): segment is Extract<AssistantComposerSegment, { type: "image" }> =>
        segment.type === "image"
    )
    .filter(segment => {
...
function getAssistantComposerAnnotations(segments: AssistantComposerSegment[]) {
  ...
  return segments
    .filter(
      (
        segment
      ): segment is Extract<AssistantComposerSegment, { type: "annotation" }> =>
        segment.type === "annotation"
    )
    .filter(segment => {
...
function getAssistantComposerVisualReferences(
  segments: AssistantComposerSegment[]
) {
...
function CanvasAssistantPanel({
  ...
  const [composerSegments, setComposerSegments] = useState<
    AssistantComposerSegment[]
  >(() => [createAssistantTextSegment("")]);
  const [draggingComposerSegmentId, setDraggingComposerSegmentId] = useState<
    string | null
  >(null);
  ...
  const [composerTextWidths, setComposerTextWidths] = useState<
  ...
  const [agentMenuStyle, setAgentMenuStyle] =
  ...
  const [messages, setMessages] = useState<CanvasAssistantMessage[]>(() => {
    ...
    if (stored.length === 1 && stored[0]?.id === "assistant-seed-1")
      return [createCanvasAssistantSeedMessage()];
  ...
  const updateAgentMenuPosition = useCallback(() => {
    ...
    const estimatedHeight = Math.min(
      248,
      measuredHeight || 112 + modelOptionCount * 36
    );
    const spaceAbove = rect.top - margin - gap;
    const spaceBelow = window.innerHeight - rect.bottom - margin - gap;
    ...
    const maxHeight = Math.max(
      160,
      Math.min(280, openAbove ? spaceAbove : spaceBelow)
    );
    const left = Math.min(
      Math.max(margin, rect.left),
      window.innerWidth - width - margin
    ...
    const top = openAbove
      ...
      : Math.min(
          window.innerHeight - margin - Math.min(estimatedHeight, maxHeight),
          rect.bottom + gap
        );
    setAgentMenuStyle({
      position: "fixed",
    ...
    (event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      const handleMove = (moveEvent: PointerEvent) => {
        ...
        const nextWidth = Math.max(
          bounds.min,
          Math.min(bounds.max, startWidth + startX - moveEvent.clientX)
        );
  ...
  const focusComposerSegment = useCallback(
    (segmentId?: string | null) => {
      window.setTimeout(() => {
        const id =
          segmentId ||
          activeComposerSegmentIdRef.current ||
          composerSegments.find(segment => segment.type === "text")?.id;
        ...
        if (input) {
          ...
          if (
            ...
            range.setStart(textNode || input, textNode ? nextPosition : 0);
            range.collapse(true);
  ...
  const focusLeadingComposerSegment = useCallback(() => {
    const firstTextSegment = composerSegments.find(
  ...
  useEffect(() => {
    ...
    composerSegments.forEach(segment => {
      ...
      nextWidths[segment.id] = Math.min(
        520,
        Math.max(32, Math.ceil(measure.scrollWidth) + 18)
      );
    });
    setComposerTextWidths(previous => {
      ...
      if (
        ...
        nextKeys.every(key => previous[key] === nextWidths[key])
      )
        return previous;
      return nextWidths;
    });
  ...
  const rememberComposerCursor = useCallback(
    (
      ...
      if (!selection || selection.rangeCount === 0) {
        activeComposerCursorRef.current = (target.textContent || "").length;
        return;
      }
      ...
      activeComposerCursorRef.current = prefix.toString().length;
    },
    []
  );

  const removeComposerSegmentsByIds = useCallback(
    (ids: string[]) => {
      ...
      setComposerSegments(prev =>
        normalizeAssistantComposerSegments(
          prev.filter(segment => !selected.has(segment.id))
        )
      );
  ...
  const moveComposerTokenSegment = useCallback(
    (
      sourceId: string,
      targetId: string,
  ...
  const handleComposerTokenDragStart = useCallback(
    (event: React.DragEvent<HTMLElement>, segmentId: string) => {
      setComposerBoxSelection(null);
      ...
      event.dataTransfer.setData(
        "application/x-artx-composer-token",
        segmentId
      );
    },
    []
  ...
  const handleComposerSegmentDragOver = useCallback(
    (event: React.DragEvent<HTMLElement>, segmentId: string) => {
      if (!draggingComposerSegmentId || draggingComposerSegmentId === segmentId)
        return;
      event.preventDefault();
      event.stopPropagation();
  ...
  const handleComposerSegmentDrop = useCallback(
    (
      ...
      event.stopPropagation();
      const sourceId =
        event.dataTransfer.getData("text/plain") || draggingComposerSegmentId;
  ...
  const getComposerSegmentSelectionText = useCallback(
    (ids: string[]) => {
      if (ids.length === 0) return "";
      const selected = new Set(ids);
  ...
  useEffect(() => {
    if (
      ...
      composerBoxSelection.active ||
      composerBoxSelection.selectedIds.length === 0
    )
      return;
    const handleCopy = (event: ClipboardEvent) => {
      const selectedText = getComposerSegmentSelectionText(
        composerBoxSelection.selectedIds
      );
  ...
  const handleComposerShellDragOver = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      if (!isComposerTokenDragEvent(event)) return;
      event.preventDefault();
      event.stopPropagation();
      event.dataTransfer.dropEffect = draggingComposerSegmentId
        ? "move"
        : "none";
    },
  ...
  const handleComposerShellDragLeave = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      if (!isComposerTokenDragEvent(event)) return;
  ...
  const handleComposerShellDrop = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      if (!isComposerTokenDragEvent(event)) return;
      event.preventDefault();
      event.stopPropagation();
  ...
  const handleReferenceSelectionApply = useCallback(
    (message: CanvasAssistantMessage) => {
      ...
      selected.forEach(item =>
        merged.set(item.id, { id: item.id, title: item.title, src: item.src })
      );
  ...
  const insertComposerToken = useCallback(
    (createTokenSegment: () => AssistantComposerSegment) => {
  ...
  useEffect(() => {
    const newAnnotations = annotationReferences.filter(
      annotation => !syncedAnnotationIdsRef.current.has(annotation.id)
    );
  ...
  const runAssistantCapability = async (
    module: string,
    instruction: string
  ) => {
      ...
      return [
        ...prev,
        {
          ...
          content: [
            ...
            activeSkill.canvasSizes?.length
              ? `适合尺寸：${activeSkill.canvasSizes.slice(0, 4).join("、")}`
              : "",
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      const detail = (
        event as CustomEvent<CanvasAssistantMessage["imageBackup"]>
      ).detail;
    ...
    return () =>
      window.removeEventListener("ai-image-generated-backup", handler);
  }, [collapsed]);
  ...
  const handleImageBackupDoubleClick = (
    ...
    window.dispatchEvent(
      new CustomEvent("ai-image-backup-activate", {
        detail: { ...backup, src: getCanvasRenderableImageSrc(backup.src) },
      })
    );
  };

  const handleRegenerateImageFromMessage = async (
    message: CanvasAssistantMessage
  ) => {
        ...
        try {
          ...
          if (decision.mode === "image") {
            ...
            const imagePayload: ImageGeneratorPayload = {
              projectId,
              prompt: imagePrompt,
              model: ALL_AI_MODEL_OPTIONS.some(
                model => model.id === payload.model
              )
                ? payload.model!
                : assistantImageModel.id,
  ...
  useEffect(() => {
    ...
    setComposerSegments(prev =>
      getAssistantComposerText(prev)
        ? prev
        : [createAssistantTextSegment("我需要帮助解决：")]
    );
    focusComposerSegment();
  ...
  async function handleSubmit() {
    ...
    const submittedVisualReferences =
      getAssistantComposerVisualReferences(composerSegments);
    const userMessage = {
      id: `user-${Date.now()}`,
      role: "user" as const,
      content: submittedText,
    ...
    const hasVisualReferences =
      submittedImages.length > 0 || submittedAnnotations.length > 0;
    const annotationContext = submittedAnnotations
      .map(
        (ann, index) =>
    ...
    try {
      ...
      const shouldReplyWithText =
        ...
        (!assistantAutoMode ||
          (decision?.mode !== "image" &&
            decision?.mode !== "reference_search" &&
            !hasAnnotationReferences));
      ...
      if (decision?.mode === "reference_search" && !hasAnnotationReferences) {
        ...
        const result = await searchReferenceImages({
          query: searchQuery,
          limit: 10,
        });
        setMessages(prev => [
          ...prev,
          {
            ...
            content:
              decision.followUp ||
              `我先帮你抓了 ${result.images.length} 张「${searchQuery}」参考图，你先选几张最接近你想法的方向，我再继续追问或直接出图。`,
            timestamp: new Date(),
    ...
    >
      ...
      {!collapsed && (
        <>
            ...
            <div className="flex flex-col gap-4">
                  ...
                  >
                    <div className="max-w-[86%]">
                      ...
                      <div
                        ...
                        onDoubleClick={
                          backup
                            ? () => handleImageBackupDoubleClick(backup)
                            : undefined
                        }
                        title={
                          backup
                      ...
                      >
                        {backup ? (
                          <div className="flex flex-col gap-2">
                            ...
                            <p
                              className="type-caption"
                              style={{ color: sub, lineHeight: "16px" }}
                            >
                              双击气泡可定位或找回图片
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col" style={{ gap: 8 }}>
                            ...
                            {message.referenceOptions &&
                              message.referenceOptions.length > 0 && (
                                <div className="flex flex-col gap-3">
                                        ...
                                        <button
                                          ...
                                          style={{
                                            ...
                                            background: active
                                              ...
                                              : "transparent",
                                            boxShadow: active
                                              ? "0 0 0 2px rgba(197,237,71,0.14)"
                                              : "none",
                                          }}
                                          onClick={() =>
                                  ...
                                  <div className="flex items-center justify-between gap-2">
                                    <span
                                      ...
                                      style={{ color: sub }}
                                    >
                                      已选 {selectedReferenceIds.length}{" "}
                                      张参考图
                                    </span>
                                    <button
                                      type="button"
                                      ...
                                      onClick={() =>
                                        handleReferenceSelectionApply(message)
                                      }
                                    >
                                      确认参考图
                                    </button>
              ...
              {isSubmitting && (
                <div className="flex justify-start">
                  <div
                    className="max-w-[86%] rounded-[var(--radius-lg-design)] px-4 py-3"
                    style={{
                      background: chipBg,
                      border: `1px solid ${border}`,
                      color: sub,
          ...
          <div className="shrink-0 px-3 pb-4 pt-1 mt-auto">
            <div
            ...
            >
              ...
              <div
                ...
                onMouseDown={event => {
                  ...
                  if (
                    target.closest(
                      "[data-composer-token], input, textarea, [contenteditable='true']"
                    )
                  )
                    return;
                  const rect = event.currentTarget.getBoundingClientRect();
                ...
                {composerSegments.map(segment => {
                  ...
                  const textWidth =
                    ...
                    (segment.text
                      ? Math.min(
                          520,
                          Math.max(32, segment.text.length * 13 + 18)
                        )
                      ...
                      <textarea
                        ...
                        onFocus={() => {
                          ...
                          activeComposerCursorRef.current =
                            input instanceof HTMLTextAreaElement ||
                            input instanceof HTMLInputElement
                              ? input.selectionStart ?? segment.text.length
                              : segment.text.length;
                          ...
                          <div className="grid grid-cols-2 gap-1 px-1 pb-1">
                            {[
                              ...
                              <button
                                ...
                                style={{
                                  color:
                                    assistantModelTab === tab.id ? text : sub,
                                  background:
                                    assistantModelTab === tab.id
                                      ? "rgba(197,237,71,0.14)"
                                      : "transparent",
              ...
              {isSubmitting && (
                <div
                  className="mt-2 flex items-center justify-between rounded-[var(--radius-md-design)] px-2 py-1.5"
...
function CanvasSearchBar({
  ...
  const visibleProjectResults = (
    historyProjectResults.length ? historyProjectResults : projectResults
  ).slice(0, 4);
  const assetResults = GENERATED_ASSETS.filter(asset => {
    const haystack =
      `${asset.title} ${asset.type} ${(asset.tags || []).join(" ")}`.toLowerCase();
    return normalized ? haystack.includes(normalized) : true;
  }).slice(0, 4);

  const bg = isDark ? "rgba(22,22,30,0.80)" : "rgba(255,255,255,0.82)";
  const panelBg = isDark
    ? "oklch(0.15 0.018 270 / 0.98)"
    : "oklch(0.995 0.002 80 / 0.98)";
  const border = isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.10)";
  const divider = isDark ? "oklch(1 0 0 / 8%)" : "oklch(0 0 0 / 8%)";
    ...
    >
      <div
        ...
        style={{
          height: 38,
          background: bg,
          border: `1px solid ${open ? "oklch(0.62 0.22 290 / 0.55)" : border}`,
          backdropFilter: "blur(14px)",
      ...
      >
        <Search
          size={14}
          style={{
            color: open ? "oklch(0.72 0.18 290)" : sub,
            flexShrink: 0,
      ...
      {open && (
        ...
        >
          <div
            ...
            style={{ borderBottom: `1px solid ${divider}` }}
          >
            <span className="type-caption uppercase" style={{ color: sub }}>
              项目
            </span>
          </div>
          {visibleProjectResults.length ? (
            visibleProjectResults.map(project => (
              <button
                key={project.id}
                className="flex items-center gap-2.5 w-full px-3 py-2.5 text-left type-caption transition-colors"
                style={{ color: text }}
                ...
                onClick={() => {
                  onProjectRequest(project);
                  setOpen(false);
                }}
              >
                <LayoutGrid
                  size={13}
                  style={{ color: "oklch(0.62 0.18 290)", flexShrink: 0 }}
                />
                <span className="min-w-0 flex-1">
                  ...
                  <span
                    ...
                    style={{ color: sub, fontSize: 11 }}
                  >
                    {project.subtitle || project.updatedAt}
                  </span>
                </span>
            ...
            <div className="px-3 py-3 type-caption" style={{ color: sub }}>
              未找到匹配项目
            </div>
          )}
          ...
          {assetResults.length ? (
            assetResults.map(asset => (
              ...
              >
                <div
                  className="w-9 h-9 rounded-[var(--radius-md-design)] overflow-hidden flex items-center justify-center"
                  style={{
                    background: isDark
                      ? "oklch(0.20 0.004 270)"
                      : "oklch(0.88 0.004 270)",
                    border: `1px solid ${divider}`,
                    flexShrink: 0,
                  }}
                >
                ...
                <div className="min-w-0 flex-1">
                  <p className="type-caption truncate" style={{ color: text }}>
                    {asset.title}
                  </p>
                  <p
                    className="type-caption truncate"
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "1px solid oklch(0.62 0.22 290 / 0.35)",
                    color: isDark
                      ? "oklch(0.82 0.18 290)"
                      : "oklch(0.42 0.18 290)",
                    flexShrink: 0,
...
function InnerCanvas({ projectId = "p1" }: { projectId?: string }) {
  ...
  const initialProjectMeta = useMemo(() => {
    if (typeof window === "undefined") return null;
    return (
      readWorkspaceProjectHistory().find(project => project.id === projectId) ||
      null
    );
  }, [projectId]);
  const projectInitialNodes = useMemo(
    () =>
      normalizeCanvasFrameNodes(
        restoredCanvasState?.nodes ||
          createInitialSocialArtboardNode(initialProjectMeta)
      ),
  ...
  const [edges, setEdges, onEdgesChange] = useEdgesState(
    restoredCanvasState?.edges || initialEdges
  );
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      const detail = (
        event as CustomEvent<{
          resolve?: (assets: ImageGeneratorReferenceAsset[]) => void;
        }>
      ).detail;
      void (async () => {
      const selectedAssetIds = new Set(
        nodesRef.current
          .filter(node => node.selected && node.type === "asset")
          .map(node => node.id)
      );
      const assetNodes = nodesRef.current
        .filter(node => node.type === "asset")
        .sort((a, b) => {
    ...
    return () =>
      window.removeEventListener("image-generator-reference-request", handler);
  }, []);
  ...
  const readCrossCanvasClipboard = useCallback(() => {
    ...
    try {
      ...
      return Array.isArray(parsed.nodes)
        ? parsed.nodes.filter(isCrossCanvasCopyNode)
        : [];
    } catch {
      return [] as Node[];
    }
  ...
  useEffect(() => {
    const clearCrossCanvasPastePriority = () => {
      preferCrossCanvasPasteRef.current = false;
    };
    window.addEventListener("focus", clearCrossCanvasPastePriority);
    document.addEventListener("visibilitychange", clearCrossCanvasPastePriority);
    return () => {
      window.removeEventListener("focus", clearCrossCanvasPastePriority);
      document.removeEventListener(
        "visibilitychange",
        clearCrossCanvasPastePriority
      );
    };
  }, []);
  ...
  const [rotateEditorState, setRotateEditorState] = useState<{
    nodeId: string;
    imageSrc: string;
  } | null>(null);
  ...
  const [annotationReferences, setAnnotationReferences] = useState<
    AnnotationReference[]
  >([]);
  const mergeReferencedAssets = useCallback(
    (assets: ImageGeneratorReferenceAsset[]) => {
      setReferencedAssets(assets);
    },
    []
  );
  const getLatestAssetNode = useCallback(
    (nodeId: string) =>
  ...
  const getLatestAssetImageSource = useCallback(
    (nodeId: string) => {
  ...
  const getVisibleAssetImageSource = useCallback(
    async (nodeId: string) => {
      const latestNode = getLatestAssetNode(nodeId);
      return latestNode ? getVisibleAssetImageSourceFromNode(latestNode) : "";
    },
    [getLatestAssetNode]
  );
  const bakeVisibleAssetNodeForCopy = useCallback(
    async (node: Node) => {
      ...
      const visibleSrc = await getVisibleAssetImageSource(node.id);
      if (!visibleSrc) return node;
  ...
  const prepareCrossCanvasCopyNodes = useCallback(
    async (nodesToCopy: Node[]) =>
      Promise.all(nodesToCopy.map(node => bakeVisibleAssetNodeForCopy(node))),
    [bakeVisibleAssetNodeForCopy]
  );
  const ensureBackgroundImageGeneration = useCallback(
    (task: PersistedImageGenerationTask | ImageGeneratorPayload) => {
      ...
      const startTask = async () => {
        try {
          if (!(task as PersistedImageGenerationTask).backgroundStartedAt) {
            markImageGenerationTaskBackgroundStarted(
              taskProjectId,
              generationId
            );
              ...
              dispatchImageGenerationTask(
                {
                  ...
                  projectId: taskProjectId,
                  status: "failed",
                  error: result.error || AI_GENERATION_NETWORK_ERROR_MESSAGE,
                },
                taskProjectId
              );
              return;
  ...
  useEffect(() => {
    ...
    const projectMeta =
      readWorkspaceProjectHistory().find(project => project.id === projectId) ||
      null;
    const restoredNodes = normalizeCanvasFrameNodes(
      saved?.nodes || createInitialSocialArtboardNode(projectMeta)
    );
    ...
    setSelectedNodeIds(
      restoredNodes.filter(node => node.selected).map(node => node.id)
    );
    updateWorkspaceProjectHistory(
      projectId,
      getCanvasProjectHistoryPatch(restoredNodes, updatedAt)
    );
    ...
    hydrateCanvasNodeImagePayloads(projectId, restoredNodes).then(
      hydratedNodes => {
  ...
  useEffect(() => {
    if (
      typeof window === "undefined" ||
      !didHydrateCanvasStateRef.current ||
  ...
  const [downloadFormat, setDownloadFormat] =
    useState<CanvasDownloadFormat>("png");
  const [assetMorePanel, setAssetMorePanel] = useState<{
    command: string;
    nodeId: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      sessionStorage.getItem("artx:activate-help-prompt") === "1" ||
      new URLSearchParams(window.location.search).get("help") === "1"
    ) {
      window.setTimeout(handleHelpPrompt, 120);
    }
    return () =>
      window.removeEventListener("artx:activate-help-prompt", handleHelpPrompt);
  }, []);
  ...
  const altDragOriginRef = useRef<Map<string, { x: number; y: number }>>(
    new Map()
  );
  const isAltDragRef = useRef(false);
  const dragStartPositionRef = useRef<Map<string, { x: number; y: number }>>(
    new Map()
  );
  const frameLiveDragPositionRef = useRef<
    ...
    async ({
      ...
      const latestSourceNode =
        sourceNode.type === "asset"
          ? getLatestAssetNode(sourceNode.id) || sourceNode
          : sourceNode;
      const sourceDisplaySize = getCanvasNodeSize(latestSourceNode);
      const resolvedDisplayW = Math.max(
        1,
        Math.round(
          preserveSourceDisplaySize
            ? sourceDisplaySize.width
            : (displayW ?? sourceDisplaySize.width)
        )
      );
      const resolvedDisplayH = Math.max(
        1,
        Math.round(
          preserveSourceDisplaySize
            ? sourceDisplaySize.height
            : (displayH ?? sourceDisplaySize.height)
        )
      );
      const generationId =
        providedGenerationId ||
        `${style}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
      const transparentLayerStyles = new Set([
        "去背景结果",
        "主体层",
        "中景层",
        "扩展结果",
      ]);
      const visibleSourceImageSrc =
        latestSourceNode.type === "asset"
          ? await getVisibleAssetImageSource(latestSourceNode.id)
          : "";
      ...
      try {
        ...
        if (providerTaskIds.length > 0) {
          toast("AI 任务 ID 已返回", {
            description: providerTaskIds.join(", "),
          });
        ...
        dispatchImageGenerationTask(
          {
            ...payload,
            status: "completed",
            images: result.images.slice(0, payload.count),
          },
          projectId
        );
        return true;
  ...
  const focusGeneratedImageNode = useCallback(
    (nodeId: string) => {
  ...
  // 预览鼠标位置（用于显示待确认的连线）
  const [penCursorPos, setPenCursorPos] = useState<{
    x: number;
    y: number;
  } | null>(null);
  // 当前正在拖拽的手柄信息
  const penDragHandleRef = useRef<{
    startX: number;
    startY: number;
    anchorIdx: number;
  } | null>(null);
  ...
  const drawParamsRef = useRef<{ color: string; width: number }>({
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent<{ color: string; width: number }>)
        .detail;
      if (detail?.color)
        drawParamsRef.current = { color: detail.color, width: detail.width };
    };
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent<{ shapeType: string; label: string }>)
        .detail;
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (
        e as CustomEvent<{ annotation: Annotation & { nodeId: string } }>
      ).detail;
  ...
  const updateGlobalAnnotation = useCallback(
  ...
  const getAnnotationReferenceFromId = useCallback(
    (id: string, textOverride?: string): AnnotationReference | null => {
      const ann = globalAnnotations.find(item => item.id === id);
      if (!ann) return null;
      const node = nodesRef.current.find(
        item => item.id === ann.nodeId && item.type === "asset"
      );
      if (!node) return null;
      const data = node.data as Record<string, unknown>;
      ...
      return {
        ...
        x: ann.x,
        y: ann.y,
        text: (typeof textOverride === "string"
          ? textOverride
          : ann.text
        ).trim(),
      };
  ...
  const handleAnnotationAddReference = useCallback(
    (id: string, text: string) => {
      ...
      setIsAssistantCollapsed(false);
      toast("已加入注释引用", {
        description: reference.text || reference.title,
      });
    },
    [getAnnotationReferenceFromId]
  );

  const handleAnnotationAiEdit = useCallback(
    async (id: string, text: string) => {
      ...
      const sourceNode = nodesRef.current.find(
        item => item.id === reference.nodeId && item.type === "asset"
      );
      if (!sourceNode) {
  ...
  const cloneNodesForHistory = useCallback(
    (items: Node[]) =>
      items.map(node => ({
        ...node,
  ...
  const clearAssetCommandState = useCallback(
    (data: Record<string, unknown>) => ({
      ...data,
      isCropping: false,
      isErasing: false,
  ...
  const clearInactiveAssetCommands = useCallback(
    (keepIds: string[] = []) => {
      ...
      setAssetMorePanel(current =>
        current && keep.has(current.nodeId) ? current : null
      );
  ...
  const cloneEdgesForHistory = useCallback(
    (items: Edge[]) =>
      items.map(edge => ({
        ...edge,
        data: edge.data
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("ai-image-backup-activate", handler);
  }, [
    createGeneratedImageNode,
    fitView,
    focusGeneratedImageNode,
    pushHistory,
    screenToFlowPosition,
    setNodes,
  ]);
  ...
  const undoCanvas = useCallback(() => {
    ...
    toast("已回退一步", {
      description: `还可回退 ${historyRef.current.length} 步`,
    });
  }, [
    cloneEdgesForHistory,
    cloneNodesForHistory,
    normalizeAssetNodeSize,
    setEdges,
    setNodes,
  ]);
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      isAssetResizingRef.current = Boolean(
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (
        e as CustomEvent<{ id: string; width: number; height: number }>
      ).detail;
      if (!detail?.id || isRestoringRef.current) return;
      // 尺寸调整前先入历史（传入当前快照）
      pushHistory(nodesRef.current, edgesRef.current);
      setNodes(nds => {
        const frame = nds.find(
          n => n.id === detail.id && n.type === "canvasFrame"
        );
        if (!frame) return nds;
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (!target) return;
      const currentTitle =
        ((target.data as Record<string, unknown>).title as
          | string
          | undefined) || "画板";
    ...
    return () =>
      window.removeEventListener("canvas-frame-title-change", handler);
  }, [pushHistory, setNodes]);
    ...
    const handler = (e: Event) => {
      ...
      setNodes(nds => {
        ...
        const historyNodes = nds.map(n =>
          n.id === detail.nodeId &&
          typeof detail.startNodeX === "number" &&
          typeof detail.startNodeY === "number" &&
          typeof detail.startW === "number" &&
          typeof detail.startH === "number"
            ? {
                ...n,
                position: { x: detail.startNodeX, y: detail.startNodeY },
                style: {
                  ...n.style,
                  width: detail.startW,
                  height: detail.startH,
                },
                data: {
                  ...(n.data as Record<string, unknown>),
                  imgW: detail.startW,
        ...
        return nds.map(n =>
          n.id === detail.nodeId
            ? {
                ...n,
                position:
                  typeof detail.nextX === "number" &&
                  typeof detail.nextY === "number"
                    ? { x: detail.nextX, y: detail.nextY }
                    : n.position,
                style: { ...n.style, width: detail.newW, height: detail.newH },
                data: { ...n.data, imgW: detail.newW, imgH: detail.newH },
              }
            : n
        );
      });
  ...
  useEffect(() => {
    const commitHandler = (e: Event) => {
      ...
      const fallbackW = Math.max(
        1,
        Math.round(detail.startW * (detail.cropRect.w / 100))
      );
      const fallbackH = Math.max(
        1,
        Math.round(detail.startH * (detail.cropRect.h / 100))
      );
      const applyCrop = (
        croppedSrc?: string,
        nextW = fallbackW,
      ...
      image.onload = () => {
        const sx = Math.max(
          0,
          Math.round((detail.cropRect.x / 100) * image.naturalWidth)
        );
        const sy = Math.max(
          0,
          Math.round((detail.cropRect.y / 100) * image.naturalHeight)
        );
        const sw = Math.max(
          1,
          Math.round((detail.cropRect.w / 100) * image.naturalWidth)
        );
        const sh = Math.max(
          1,
          Math.round((detail.cropRect.h / 100) * image.naturalHeight)
        );
  ...
  useEffect(() => {
    const applyHandler = async (e: Event) => {
      ...
      if (!detail?.nodeId || !detail.imageSrc || !detail.editedText.trim())
        return;
      const sourceNode = nodesRef.current.find(
        n => n.id === detail.nodeId && n.type === "asset"
      );
    ...
    return () =>
      window.removeEventListener("asset-text-edit-apply", applyHandler);
      ...
      setNodes(nds => {
        ...
        return nds.map(n => {
          ...
          return {
            ...n,
            position: {
              x: n.position.x + detail.dx,
              y: n.position.y + detail.dy,
            },
            style: { ...n.style, width: detail.newW, height: detail.newH },
            data: {
              ...n.data,
  ...
  const handleNodesChangeWithHistory = useCallback(
    (changes: Parameters<typeof onNodesChange>[0]) => {
      ...
      const isOnlyAssetSizeSync = effectiveChanges.every(
      ...
      if (!isRestoringRef.current && !isAssetResizingRef.current) {
        const hasNonDragChange = effectiveChanges.some(
          change =>
            change.type !== "select" &&
  ...
  const areNodesGrouped = useCallback(
    (ids: string[]) => {
      ...
      return (
        selected.length > 0 &&
        selected.every(n =>
          Boolean((n.data as Record<string, unknown>).groupId)
        )
      );
    },
    [nodes]
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      const actionIds = selectedNodeIds.includes(detail.nodeId)
        ? selectedNodeIds
        : [detail.nodeId];
      clearInactiveAssetCommands(actionIds);
      setSelectedNodeIds(actionIds);
      setNodes(nds =>
        nds.map(n => ({ ...n, selected: actionIds.includes(n.id) }))
      );
      setNodeCtxMenu({
        x: detail.x,
        y: detail.y,
  ...
  const handleNodeAction = useCallback(
    async (action: string, nodeId: string) => {
      ...
      if (action === "delete") {
        ...
        setEdges(eds =>
          eds.filter(
            e => !actionIds.includes(e.source) && !actionIds.includes(e.target)
          )
        );
        setSelectedNodeIds([]);
      } else if (action === "duplicate") {
        const source = (
          await prepareCrossCanvasCopyNodes(
            nodes.filter(n => actionIds.includes(n.id))
          )
        ...
        const now = Date.now();
        const groupMap = new Map<string, string>();
        const duplicated = source.map((node, index) => {
          const id = `${node.type}-duplicate-${now}-${index}`;
          const data = { ...(node.data as Record<string, unknown>) };
          const oldGroupId =
            typeof data.groupId === "string" ? data.groupId : undefined;
          const groupId = oldGroupId
        ...
        setNodes(nds =>
          nds.map(n =>
            actionIds.includes(n.id)
              ? {
                  ...n,
                  data: { ...(n.data as Record<string, unknown>), groupId },
                }
              : n
          )
        );
        toast(`已创建「${autoName}」`, {
          description: `${actionIds.length} 个图片节点已打组，右键容器可管理`,
        });
      } else if (action === "ungroup") {
        pushHistory();
        // Find groupId from the action nodes
        const ungroupIds =
          actionIds.length > 0 ? actionIds : nodeCtxMenu?.selectedIds || [];
        const targetNode = nodes.find(n => ungroupIds.includes(n.id));
        const gid = targetNode
          ? ((targetNode.data as Record<string, unknown>).groupId as
              | string
              | undefined)
          : undefined;
        setNodes(nds =>
          nds.map(n => {
            const nGroupId = (n.data as Record<string, unknown>).groupId as
              | string
              | undefined;
        ...
        const selectedAssetIds = actionIds.filter(id =>
          nodes.some(n => n.id === id && n.type === "asset")
        );
        if (selectedAssetIds.length < 2) {
          toast("请至少选择 2 个图片节点再自动布局");
          return;
        }
        pushHistory();
        ...
        const unselectedBounds = nodes
          .filter(n => !selectedAssetIds.includes(n.id))
          .map(getCanvasNodeBounds);

        const buildLayout = (top: number) =>
          selected.map((node, index) => {
            const row = Math.floor(index / columns);
            const col = index % columns;
            const size = sizes[index];
            const x =
              startX + col * cellWidth + (cellWidth - gap - size.width) / 2;
            const y =
              top + row * cellHeight + (cellHeight - gap - size.height) / 2;
            return {
              id: node.id,
              position: { x, y },
              bounds: {
        ...
        while (
          attempts < 80 &&
          layout.some(item =>
            unselectedBounds.some(bounds =>
              canvasRectsOverlap(item.bounds, bounds, 12)
            )
          )
        ) {
          startY += cellHeight;
        ...
        const positionById = new Map(
          layout.map(item => [item.id, item.position])
        );
        setNodes(nds =>
          nds.map(n =>
            selectedAssetIds.includes(n.id)
              ? {
                  ...n,
        ...
        toast("已完成自动布局", {
          description: `${selectedAssetIds.length} 个图片节点已重新排列，节点之间不重叠`,
        });
        ...
        setSelectedNodeIds(selectedAssetIds);
        toast("已完成横向排列", {
          description: `${selectedAssetIds.length} 个图片节点已从左到右排列，垂直居中对齐，总宽 ${Math.round(totalWidth)}px`,
        });
        ...
        const selectedFrameNodes = nodes.filter(
          n => actionIds.includes(n.id) && n.type === "canvasFrame"
        );
        if (selectedAssetNodes.length === 0) {
          if (selectedFrameNodes.length === 1) {
            ...
            ).__artx_frame_download__ = selectedFrameNodes[0];
            (
              window as unknown as Record<string, unknown>
            ).__artx_download_nodes__ = undefined;
            (
              window as unknown as Record<string, unknown>
            ).__artx_single_download__ = undefined;
            return;
          }
          if (selectedFrameNodes.length > 1) {
            toast("一次只能导出一个画板", {
              description: "请只选中一个画板后再导出",
            });
            return;
          }
          toast("没有可下载的图片节点");
        ...
        if (selectedAssetNodes.length === 1) {
          ...
          const src = await getVisibleAssetImageSource(node.id);
          if (!src) {
            toast("该节点没有可下载的图片");
            return;
          }
          ...
          (
            window as unknown as Record<string, unknown>
          ).__artx_single_download__ = { title, src, node };
          (
            window as unknown as Record<string, unknown>
          ).__artx_download_nodes__ = undefined;
          (
            window as unknown as Record<string, unknown>
          ).__artx_frame_download__ = undefined;
        } else {
          // 多张直接弹出格式选择对话框打包下载
          ...
          (
            window as unknown as Record<string, unknown>
          ).__artx_download_nodes__ = selectedAssetNodes;
          (
            window as unknown as Record<string, unknown>
          ).__artx_single_download__ = undefined;
          (
        ...
        setNodes(nds => {
          const topZ =
            Math.max(
              0,
              ...nds.map(n => (typeof n.zIndex === "number" ? n.zIndex : 0))
            ) + 1;
          const actionIdSet = new Set(actionIds);
          const updated = nds.map(n =>
            actionIdSet.has(n.id)
              ? {
                  ...n,
                  zIndex: topZ,
                  data: {
                    ...(n.data as Record<string, unknown>),
                    note:
                      ((n.data as Record<string, unknown>).note as string) ||
                      "",
                    noteOpen: true,
                    noteEditing: true,
                  },
        ...
        const node =
          getLatestAssetNode(nodeId) || nodes.find(n => n.id === nodeId);
  ...
  const addNode = useCallback(
    (_type: string, x: number, y: number) => {
      pushHistory();
      const id = `asset-${Date.now()}`;
      setNodes(nds => [
        ...nds,
  ...
  const handleAssetAddFromSearch = useCallback(
  ...
  useEffect(() => {
    const handleAssetRegenerateRequest = (event: Event) => {
      const detail = (event as CustomEvent<ImageRegenerateRequestDetail>)
        .detail;
            ...
            .catch(error => {
              const message =
                error instanceof Error
                  ? error.message
                  : AI_GENERATION_NETWORK_ERROR_MESSAGE;
              dispatchImageGenerationTask(
                { ...payload, status: "failed", error: message },
                projectId
              );
      ...
      const sourceNode = nodesRef.current.find(
        node => node.id === detail.nodeId
      );
      const sourceSize = sourceNode
  ...
  useEffect(() => {
    const handleImageGenerate = (event: Event) => {
      ...
      const center = rect
        ...
        : { x: 120, y: 80 };
      const size =
        detail.displaySize || getImageDisplaySizeForRatio(detail.ratio);
            ...
            return {
              ...
              data: {
                ...
                title: detail.titleBase
                  ? `${detail.titleBase}${images.length > 1 ? ` ${index + 1}` : ""}`
                  : `生成图像 · ${detail.style}${images.length > 1 ? ` ${index + 1}` : ""}`,
                assetType: "AI 生成",
                tags: [
                  detail.model,
                  detail.ratio,
      ...
      backupItems.forEach(item => {
        window.dispatchEvent(
          new CustomEvent("ai-image-generated-backup", { detail: item })
        );
      });
      markImageGenerationTaskConsumed(projectId, generationId);
    ...
    return () =>
      window.removeEventListener("image-generator-submit", handleImageGenerate);
  }, [
    edgesRef,
    ensureBackgroundImageGeneration,
    projectId,
  ...
  useEffect(() => {
    ...
    consumeCompletedImageGenerationTasks(projectId).then(completedTasks => {
      ...
      completedTasks.forEach(task => {
        window.dispatchEvent(
          new CustomEvent("image-generator-submit", {
            detail: {
              ...task,
              projectId,
              status: "completed",
  ...
  const failTimedOutImageGenerationNodes = useCallback(() => {
    ...
    setNodes(nds => {
      ...
      const nextNodes = nds.map(node => {
        if (node.type !== "asset") return node;
        const data = node.data as Record<string, unknown>;
        if (data.isGeneratingImage !== true || data.isGenerationFailed === true)
          return node;
        if (
      ...
      setNodeCtxMenu({
        ...
        nodeType:
          enteringGroupId === groupId
            ? "group-container-inside"
            : "group-container",
        selectedIds: nodes
          .filter(n => (n.data as Record<string, unknown>).groupId === groupId)
          .map(n => n.id),
        grouped: true,
      });
      // Store groupId for ungroup/enter/rename actions
      (window as unknown as Record<string, unknown>).__artx_ctx_group_id__ =
        groupId;
    },
  ...
  const handleGroupContainerDragMove = useCallback(
    (groupId: string, deltaDx: number, deltaDy: number) => {
      if (deltaDx === 0 && deltaDy === 0) return;
      setNodes(nds =>
        nds.map(n => {
          const gid = (n.data as Record<string, unknown>).groupId as
            | string
            | undefined;
          ...
          return {
            ...n,
            position: {
              x: n.position.x + deltaDx / viewport.zoom,
              y: n.position.y + deltaDy / viewport.zoom,
            },
          };
        })
      );
  ...
  const handleGroupContainerDragEnd = useCallback(
    (groupId: string, _dx: number, _dy: number) => {
      ...
      setNodes(nds =>
        nds.map(n => {
          ...
          const existingPoints =
            (n.data.points as { x: number; y: number }[]) || [];
          ...
          if (isShift && start) {
            ...
            const flowSnapped = screenToFlowPosition({
              x: snappedX + rect.left,
              y: snappedY + rect.top,
            });
            newPoint = {
              x: flowSnapped.x - origin.x,
              y: flowSnapped.y - origin.y,
            };
            // Shift 模式只保留起始点和当前点（实现直线预览）
            return {
              ...n,
              data: { ...n.data, points: [{ x: 0, y: 0 }, newPoint] },
            };
          } else {
            // 自由模式：每隔一定距离采样一个点（过滤过于密集的点）
            const flowCurrent = screenToFlowPosition({
              x: e.clientX,
              y: e.clientY,
            });
            newPoint = {
              x: flowCurrent.x - origin.x,
              y: flowCurrent.y - origin.y,
            };
            const last = existingPoints[existingPoints.length - 1];
            const distSq = last
              ? (newPoint.x - last.x) ** 2 + (newPoint.y - last.y) ** 2
              : Infinity;
            // 至少相隔 2px（flow 坐标）才采样
            if (distSq < 4 / (viewport.zoom * viewport.zoom)) return n;
            return {
              ...n,
              data: { ...n.data, points: [...existingPoints, newPoint] },
            };
          }
        })
      );
        ...
        setNodes(nds =>
          nds.map(n => {
            ...
            if (existingAnchors.length > 1) {
              ...
              const firstAbsX =
                (nodePos.x + firstAnchor.x) * viewport.zoom + viewport.x;
              const firstAbsY =
                (nodePos.y + firstAnchor.y) * viewport.zoom + viewport.y;
              const dist = Math.sqrt(
                (e.clientX - (rect.left + firstAbsX)) ** 2 +
                  (e.clientY - (rect.top + firstAbsY)) ** 2
              );
              if (dist < 12) {
                // 关闭路径
                const updated = {
                  ...n,
                  data: { ...n.data, closed: true, anchorEditMode: true },
                };
                setPenNodeId(null);
                setPenCursorPos(null);
                return updated;
  ...
  // ── 创建画板：鼠标事件处理 ──
  const handleCreateCanvasMouseDown = useCallback(
    (e: React.MouseEvent) => {
  ...
  const handleCreateCanvasMouseUp = useCallback(
    (e: React.MouseEvent) => {
      if (!isDrawingRef.current || !drawStartRef.current) return;
      const rect = containerRef.current?.getBoundingClientRect();
      if (!rect) return;
  ...
  const handleCreateCanvasConfirm = useCallback(() => {
    ...
    const selectedSocialPreset = SOCIAL_MEDIA_SIZE_PRESETS.find(
      preset => preset.id === canvasSocialPresetId
    );
    const title =
    ...
    // 将屏幕坐标转换为 flow 坐标
    const flowPos = screenToFlowPosition({
      x: (containerRef.current?.getBoundingClientRect().left || 0) + minX,
      y: (containerRef.current?.getBoundingClientRect().top || 0) + minY,
    });
      ...
      return [
        ...nds,
        {
          ...
          data: {
            ...
            bgColor,
            originalBgColor,
            socialPresetId: selectedSocialPreset?.id,
          },
        },
      ];
    });
  ...
  const handleCreateShapeConfirm = useCallback(() => {
    ...
    const opacity = Math.min(
      1,
      Math.max(0, (parseInt(shapeOpacity) || 100) / 100)
    );
  ...
  const createDroppedImageSourceNode = useCallback(
    async (src: string, index: number, origin: { x: number; y: number }) => {
      const { image: img, localSrc } = await loadExternalImageWithProxyFallback(
        src
      );
      return new Promise<Node>((resolve, reject) => {
        if (!img) {
          reject(new Error("外部图片链接加载失败"));
          return;
        }
  ...
  const addDroppedImageSources = useCallback(
    async (
      ...
      const droppedResults = await Promise.allSettled(
      ...
      console.info("[artx-external-drop-failed]", { sources: uniqueSources });
      toast("拖入图片失败", {
        description: "外部图片链接加载失败，请拖拽图片本身或右键复制图片后粘贴",
      });
      return false;
    },
  ...
  const handleCanvasDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const files = Array.from(
        new Map(
          [...Array.from(e.dataTransfer.files), ...itemFiles]
            .filter(fileLooksLikeImage)
            .map(file => [
      ...
      if (files.length === 0) {
        const dataTransferSnapshot = {
          ...
          stringItems: getDataTransferStringItems(e.dataTransfer),
        ...
        void (async () => {
          const extraEntries = await Promise.all(
            dataTransferSnapshot.stringItems.map(readDataTransferItemString)
          );
          const sources = extractImageSourcesFromDataTransfer(null, [
            ...dataTransferSnapshot.typeEntries,
            ...extraEntries,
          ]);
          if (sources.length > 0) {
          ...
          toast("请拖入图片文件或网页图片", {
            description:
              "支持 JPG、PNG、GIF、WebP 文件，也支持从网页直接拖入图片",
          });
        })();
        return;
      }
  ...
  const createClipboardImageNode = useCallback(
    (blob: Blob, index: number, origin: { x: number; y: number }) =>
      new Promise<Node>((resolve, reject) => {
        const reader = new FileReader();
  ...
  const createClipboardImageSourceNode = useCallback(
    async (src: string, index: number, origin: { x: number; y: number }) => {
      const { image: img, localSrc } = await loadExternalImageWithProxyFallback(
        src
      );
  ...
  const pasteClipboardImages = useCallback(
    async (blobs: Blob[], originOverride?: { x: number; y: number }) => {
      ...
      try {
        const nodesToPaste = await Promise.all(
        ...
        setNodes(nds =>
          nds
            ...
            .concat(nodesToPaste.map(node => ({ ...node, selected: true })))
        );
  ...
  const pasteClipboardImageSources = useCallback(
    ...
    [
      createClipboardImageSourceNode,
      pushHistory,
      screenToFlowPosition,
      setNodes,
    ]
  ...
  const pasteClipboardPayload = useCallback(
    async (
      ...
      const sources = Array.from(
        new Set([
          ...extractImageSourcesFromDataTransfer(clipboardData),
  ...
  const sanitizeDownloadName = useCallback(
    (value: string) =>
      value.replace(/[/\\:*?"<>|]/g, "_").trim() || "artx-image",
    []
  );

  const imageSrcToFormatBlob = useCallback(
    async (src: string, format: ImageDownloadFormat) => {
      if (!src) return null;
      const mimeType = format === "jpg" ? "image/jpeg" : "image/png";
      let readable: Awaited<ReturnType<typeof loadReadableImageForCanvas>> | null =
        null;
      try {
        readable = await loadReadableImageForCanvas(src);
        const image = readable.image;
        const canvas = document.createElement("canvas");
  ...
  const exportFrameAsImageBlob = useCallback(
    async (frameNode: Node, format: ImageDownloadFormat) => {
      const frameData = frameNode.data as Record<string, unknown>;
      const width = Math.max(1, Math.round((frameData.width as number) || 800));
      const height = Math.max(
        1,
        Math.round((frameData.height as number) || 600)
      );
      const canvas = document.createElement("canvas");
      canvas.width = width;
      canvas.height = height;
      ...
      const background =
        (frameData.backgroundColor as string) ||
        (frameData.bgColor as string) ||
        (frameData.fill as string);
      ...
      const embeddedAssets = nodesRef.current
        ...
        .sort(
          (a, b) => ((a.zIndex as number) || 0) - ((b.zIndex as number) || 0)
        );
      for (const assetNode of embeddedAssets) {
        const src = await getVisibleAssetImageSourceFromNode(assetNode);
        if (!src) continue;
        try {
          const readable = await loadReadableImageForCanvas(src);
  ...
  const exportFrameAsPsdBlob = useCallback(
    async (frameNode: Node) => {
      const frameData = frameNode.data as Record<string, unknown>;
      const width = Math.max(1, Math.round((frameData.width as number) || 800));
      const height = Math.max(
        1,
      ...
      for (let index = 0; index < embeddedAssets.length; index += 1) {
        ...
        try {
          ...
          try {
            ...
            layerCtx.drawImage(readable.image, 0, 0, layerWidth, layerHeight);
            compositeCtx?.drawImage(layerCanvas, left, top);
            layers.unshift({
              name:
                ((assetNode.data as Record<string, unknown>).title as string) ||
                `Layer ${index + 1}`,
              left,
              top,
  ...
  const handleBatchDownload = useCallback(
    async (
      ...
      try {
        ...
        let successCount = 0;
        await Promise.all(
          assetNodes.map(async (node, index) => {
            const data = node.data as Record<string, unknown>;
            const title = (data.title as string) || `image-${index + 1}`;
            const src = await getVisibleAssetImageSourceFromNode(node);
            if (!src) return;
        ...
        toast("下载完成", {
          description: `已将 ${successCount} 张图片打包为 ${zipName}.zip`,
        });
      } catch (err) {
        toast.dismiss(toastId);
    ...
    [imageSrcToFormatBlob, sanitizeDownloadName]
  );

  // ── Alt + drag 复制节点 ──
  // 拖拽开始时：若按下 Alt，记录被拖节点的原始位置
  const handleAltDragStart = useCallback(
    (_event: React.MouseEvent, node: Node) => {
      ...
      frameLiveDragPositionRef.current = new Map([
      ...
      dragIds.forEach(id => {
        ...
        if (n)
          altDragOriginRef.current.set(id, {
            x: n.position.x,
            y: n.position.y,
          });
      });
  ...
  const handleFrameLiveDrag = useCallback(
    (_event: MouseEvent, node: Node) => {
      if (node.type !== "canvasFrame") return;
      const previous =
        frameLiveDragPositionRef.current.get(node.id) ||
        dragStartPositionRef.current.get(node.id);
      if (!previous) {
        frameLiveDragPositionRef.current.set(node.id, {
          x: node.position.x,
          y: node.position.y,
        });
        return;
  ...
  // ── 检测图片节点是否拖入画布帧，若是则嵌入 ──
  const checkAndEmbedIntoFrame = useCallback(
  ...
  const handleAltDragStop = useCallback(
    (_event: MouseEvent, _node: Node) => {
  ...
  const handleNormalDragStop = useCallback(
    (_event: MouseEvent, node: Node) => {
      ...
      const currentFrameId = (draggedNode.data as Record<string, unknown>)
        .embeddedInFrame as string | undefined;
          ...
          const data = n.data as Record<string, unknown>;
	          if (!frame) {
	            return {
	              ...
	              data: {
	                ...data,
	                embeddedInFrame: undefined,
	                frameClipInsets: undefined,
	                frameClipActive: undefined,
	              },
              parentId: undefined,
	              extent: undefined,
	            };
	          ...
	          const frameWidth = (frameData.width as number) || 800;
	          const frameHeight = (frameData.height as number) || 600;
	          const nodeWidth = (data.imgW as number) || (n.width as number) || 200;
	          const nodeHeight = (data.imgH as number) || (n.height as number) || 200;
	          const shadowBleed = n.selected ? 14 : 12;
	          ...
	          const frameClipActive =
	            nodeLeft - shadowBleed < frameLeft ||
	            nodeTop - shadowBleed < frameTop ||
	            nodeRight + shadowBleed > frameRight ||
	            nodeBottom + shadowBleed > frameBottom;
	          const frameClipInsets = {
	            top: Math.round(
	              Math.max(nodeTop - shadowBleed, frameTop) - nodeTop
	            ),
	            right: Math.max(
	              -shadowBleed,
	              Math.round(nodeRight - Math.min(nodeRight + shadowBleed, frameRight))
	            ),
	            bottom: Math.max(
	              -shadowBleed,
	              Math.round(
	                nodeBottom - Math.min(nodeBottom + shadowBleed, frameBottom)
	              )
	            ),
	            left: Math.round(
	              Math.max(nodeLeft - shadowBleed, frameLeft) - nodeLeft
	            ),
	          };
	          return {
	            ...n,
  ...
  const handleGroupAction = useCallback(
    (action: string) => {
      const groupId = (window as unknown as Record<string, unknown>)
        .__artx_ctx_group_id__ as string | undefined;
      if (!groupId) return;
      if (action === "batch-download") {
        // 打开格式选择弹窗，让用户选择 ZIP 打包格式
        const groupNodes = nodes.filter(
          n =>
            (n.data as Record<string, unknown>).groupId === groupId &&
            n.type === "asset"
        );
        ...
        (window as unknown as Record<string, unknown>).__artx_download_nodes__ =
          groupNodes;
        (
          window as unknown as Record<string, unknown>
        ).__artx_single_download__ = undefined;
        (window as unknown as Record<string, unknown>).__artx_frame_download__ =
          undefined;
  ...
  useEffect(() => {
    const handlePaste = (event: ClipboardEvent) => {
      ...
      if (pendingCopy && preferCrossCanvasPasteRef.current) {
        void pendingCopy.then(() => {
          pendingCrossCanvasCopyRef.current = null;
          preferCrossCanvasPasteRef.current = false;
          pasteCrossCanvasClipboard();
        });
        return;
      ...
      if (
        preferCrossCanvasPasteRef.current &&
        getCrossCanvasClipboard().length > 0
      ) {
        preferCrossCanvasPasteRef.current = false;
        pasteCrossCanvasClipboard();
        return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        ...
        setEdges(eds =>
          eds.filter(
            e =>
              !selectedDeletableIds.includes(e.source) &&
              !selectedDeletableIds.includes(e.target)
          )
        );
        const assetCount = selectedDeletableIds.filter(id =>
          nodes.some(n => n.id === id && n.type === "asset")
        ).length;
        const otherCount = selectedDeletableIds.length - assetCount;
        const desc =
          assetCount > 0 && otherCount > 0
            ? `已删除 ${assetCount} 个图片、${otherCount} 个图形元素`
            : assetCount > 0
              ? `已删除 ${assetCount} 个图片节点`
              : `已删除 ${otherCount} 个图形元素`;
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "a") {
        ...
        if (selectableIds.length > 0) {
          ...
          setNodes(nds =>
            nds.map(n => ({ ...n, selected: selectableSet.has(n.id) }))
          );
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "c") {
        ...
        if (toCopy.length > 0) {
          e.preventDefault();
          const copyPromise = prepareCrossCanvasCopyNodes(toCopy).then(preparedNodes => {
            const copied = writeCrossCanvasClipboard(preparedNodes);
            const isMac =
          ...
          pendingCrossCanvasCopyRef.current = copyPromise;
          void copyPromise.finally(() => {
            if (pendingCrossCanvasCopyRef.current === copyPromise) {
              pendingCrossCanvasCopyRef.current = null;
            }
          });
        }
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "g") {
        ...
        toast(`已打组 ${selectedNodeIds.length} 个画布`, {
          description: "Windows 使用 Ctrl+G，Mac 使用 Command+G",
        });
      }
    };
  ...
  const handleLassoCut = useCallback(
    (lassoRect: LassoRect) => {
      ...
      const toFlow = (sx: number, sy: number) =>
        screenToFlowPosition({
          x: sx + containerRect.left,
          y: sy + containerRect.top,
        });
      ...
      if (cutIds.length > 0) {
        ...
        toast(`已切断 ${cutIds.length} 条连线`, {
          description: "松开 C 键退出切割模式",
        });
      } else {
  ...
  const groupOverlayData: GroupInfo[] = useMemo(() => {
    ...
    groupMap.forEach((groupNodes, groupId) => {
      const bounds = getCanvasNodesBounds(
        groupNodes,
  ...
  const selectedImageNodeIds = selectedNodeIds.filter(id =>
    nodes.some(n => n.id === id && n.type === "asset")
  );
  const selectedVisualNodeIds = selectedNodeIds.filter(id =>
    nodes.some(
      n => n.id === id && (n.type === "asset" || n.type === "canvasFrame")
    )
  );
  ...
  // 文字节点选中状态
  const selectedTextNodeIds = selectedNodeIds.filter(id =>
    nodes.some(n => n.id === id && n.type === "text")
  );
  const selectedTextNode =
    selectedTextNodeIds.length === 1
      ? nodes.find(n => n.id === selectedTextNodeIds[0])
      : null;
  const selectedTextBounds = selectedTextNode
    ? getCanvasNodesBounds(nodes, [selectedTextNode.id])
    : null;
  ...
  const handleTextNodeDownload = useCallback(() => {
    ...
    offscreen.toBlob(blob => {
      ...
      a.href = url;
      a.download = `artx-text-${Date.now()}.png`;
      a.click();
          ...
          textTransform === "uppercase"
            ...
            : textTransform === "lowercase"
              ...
              : textTransform === "capitalize"
                ? line.replace(/(^|\s)\S/g, c => c.toUpperCase())
                : line;
    ...
    b.href = svgUrl;
    b.download = `artx-text-${Date.now()}.svg`;
    b.click();
  ...
  const handleSingleImageToolbarAction = useCallback(
    async (action: string) => {
      ...
      if (!nodeId) return;
      const aiToolbarActions = new Set([
        "quick-edit",
        "upscale",
        "remove-background",
        "remove-watermark",
      ...
      setNodes(nds =>
        nds.map(n =>
          n.id === nodeId && n.type === "asset"
            ? {
                ...n,
                data: clearAssetCommandState(n.data as Record<string, unknown>),
              }
            : n
        )
      );
      ...
      if (isCanvasFrame) {
        if (
          ...
          toast("画布命令", {
            description:
              "画布节点已复用图片节点命令框架，可拖动、缩放、复制、删除与调整层级",
          });
          return;
        }
        if (
          ...
          toast("画布节点暂不支持该 AI 图片处理", {
            description: "请选择具体图片节点后使用此命令",
          });
          return;
        }
      }
      ...
      if (action === "upscale") {
        ...
        toast("HD 高清化中", {
          description: "正在生成 4K 高清图片，新图会出现在原图旁边",
        });
        await runDerivedImageGeneration({
          ...
          run: async () => {
            try {
              ...
              const shouldFallbackToImageEdit =
                /Cannot POST|non-JSON|网页内容|404|not found|未正确连接/i.test(
                  message
                );
      ...
      if (action === "remove-watermark") {
        ...
        const sourceSize = getCanvasNodeSize(targetNode);
        toast("AI 去水印中", {
          description: "正在移除图片水印，新图会出现在原图旁边",
        });
        await runDerivedImageGeneration({
          sourceNode: targetNode,
          prompt: "去水印",
      ...
      if (action === "erase") {
        ...
        window.dispatchEvent(
          new CustomEvent("tool-mode-change", { detail: { mode: "erase" } })
        );
        toast("橡皮工具", {
          description: "涂抹需要移除的区域，调整橡皮尺寸后点击立即使用",
        });
        return;
      }
      if (action === "edit-text") {
      ...
      if (action === "edit-elements") {
        const assetNode = nodesRef.current.find(
          n => n.id === nodeId && n.type === "asset"
        );
        if (!assetNode) return;
      ...
      if (action === "expand") {
        ...
        window.dispatchEvent(
          new CustomEvent("tool-mode-change", { detail: { mode: "expand" } })
        );
        toast("扩展", {
          description: "拖拽边界框扩大需要延展的范围，空白区域就是 AI 扩展区域",
        });
        return;
      }
      ...
      toast(labels[action] || "功能即将上线", {
        description: "已保留 Lovart 命令入口，后续可接入对应 AI 处理能力",
      });
    },
    [
  ...
  const handleSocialMediaSizeGenerate = useCallback(
    async (payload: SocialMediaExportPayload) => {
      if (!assetMorePanel) return;
      const imageSrc = await getVisibleAssetImageSource(assetMorePanel.nodeId);
      if (!imageSrc) {
      ...
      const toastId = toast.loading(
        `正在导出 ${outputSizes.length} 张多平台封面...`
      );
      try {
        const zip = new JSZip();
        const format = payload.format || "png";
        const defaultTransform = payload.transform || {
          offsetX: 0,
          offsetY: 0,
          scale: 1,
          rotation: 0,
	        ...
	        await Promise.all(
	          outputSizes.map(async item => {
	            const blob = await createMultiPlatformCoverBlob(
	              imageSrc,
	              { width: item.width, height: item.height },
              payload.transforms?.[item.id] || defaultTransform,
              format
	            );
	        ...
	        const zipBlob = await zip.generateAsync({ type: "blob" });
	        const zipName = sanitizeDownloadName("ArtX 多平台封面");
	        saveAs(zipBlob, `${zipName}-${Date.now()}.zip`);
        toast.dismiss(toastId);
        toast("多平台封面已导出", {
          description: `已保存 ${outputSizes.length} 张 ${format.toUpperCase()} 图片`,
        });
      } catch (error) {
        toast.dismiss(toastId);
        const message = error instanceof Error ? error.message : "请稍后重试";
    ...
    [assetMorePanel, getVisibleAssetImageSource, sanitizeDownloadName]
  );
  const handleAssetMorePanelApply = useCallback(
    async (label: string, adjustments?: AssetAdjustmentValues) => {
      if (!assetMorePanel) return;
      if (assetMorePanel.command === "vector") {
        const sourceNode = nodesRef.current.find(
          n => n.id === assetMorePanel.nodeId && n.type === "asset"
        );
        if (!sourceNode) {
          setAssetMorePanel(null);
          return;
        }
        const imageSrc = await getVisibleAssetImageSource(
          assetMorePanel.nodeId
        );
        if (!imageSrc) {
        ...
        const vectorPromptMap: Record<string, string> = {
          提取轮廓:
            "将图片转换为清晰的矢量轮廓风格结果。保留主体完整外形、关键边缘、比例和识别特征，使用干净线条与透明或简洁背景，不要改变主体姿态。",
          扁平化:
            "将图片转换为扁平化矢量插画风格结果。保留原图主体、构图比例、主要色块和品牌识别特征，减少真实纹理，使用简洁可编辑的形状语言。",
          高清矢量:
            "将图片转换为高清矢量海报风格结果。保留主体完整性、构图比例和关键细节，边缘锐利、色块干净、适合继续设计编辑。",
        };
        await runDerivedImageGeneration({
          sourceNode,
      ...
      if (assetMorePanel.command === "adjust") {
        ...
        const nextAdjustments =
          adjustments ||
          normalizeAssetAdjustments(
            data.assetAdjustmentPreview || data.assetAdjustments
          );
        try {
          const adjustedDataUrl = await applyAssetAdjustmentsToImage(
            imageSrc,
            nextAdjustments
          );
          pushHistory();
          setNodes(nds =>
            nds.map(n => {
              if (n.id !== assetMorePanel.nodeId || n.type !== "asset")
                return n;
  ...
  const selectedImageNode =
    selectedVisualNodeIds.length === 1
      ? nodes.find(
          n =>
            n.id === selectedVisualNodeIds[0] &&
            (n.type === "asset" || n.type === "canvasFrame")
        )
      : null;
  const assetMorePanelNode = assetMorePanel
    ? nodes.find(n => n.id === assetMorePanel.nodeId && n.type === "asset")
    : null;
  const assetMorePanelData = assetMorePanelNode?.data as
    | Record<string, unknown>
    | undefined;
  const assetMorePanelImageSrc = assetMorePanel
    ? getLatestAssetImageSource(assetMorePanel.nodeId)
    : "";
  const selectedImageBounds = selectedImageNode
    ? getCanvasNodeBounds(selectedImageNode)
    : getCanvasNodesBounds(nodes, selectedVisualNodeIds);
  ...
  const handleSelectedVisualToolbarContextMenu = useCallback(
    (event: React.MouseEvent) => {
      ...
      const rect = containerRef.current?.getBoundingClientRect();
      const actionIds = selectedNodeIds.includes(selectedImageNode.id)
        ? selectedNodeIds
  ...
  const displayNodesBase = nodes.map(n => {
    ...
    const embeddedFrameId =
      n.type === "asset"
        ? (nodeData.embeddedInFrame as string | undefined)
        : undefined;
    const embeddedFrame = embeddedFrameId
      ? nodes.find(
          item => item.id === embeddedFrameId && item.type === "canvasFrame"
        )
      : null;
    const frameClipInsets = embeddedFrame
      ? (() => {
          ...
          const right = Math.max(
            0,
            Math.round(assetBounds.right - frameBounds.right)
          );
          const bottom = Math.max(
            0,
            Math.round(assetBounds.bottom - frameBounds.bottom)
          );
  ...
  return (
    <div
      ...
      style={{
        ...
        background: "#222222",
        cursor:
          activeToolMode === "smart-canvas" ||
          activeToolMode.startsWith("shape-draw:") ||
          activeToolMode === "pen" ||
          activeToolMode === "draw"
            ? "crosshair"
            : undefined,
      ...
      onMouseDown={e => {
        ...
        handleCreateCanvasMouseDown(e);
      }}
      onMouseMove={e => {
        handleFreehandMouseMove(e);
        handlePenMouseMove(e);
        handleCreateCanvasMouseMove(e);
      }}
      onMouseUp={e => {
    ...
    >
      {assetMorePanel &&
        (assetMorePanel.command === "mockup" ? (
          ...
          <AssetMoreCommandPanel
            ...
            initialAdjustments={normalizeAssetAdjustments(
              assetMorePanelData?.assetAdjustmentPreview ||
                assetMorePanelData?.assetAdjustments
            )}
            imageSrc={assetMorePanelImageSrc}
            onClose={closeAssetMorePanel}
              ...
              setNodes(nds =>
                nds.map(n => {
                  if (n.id !== assetMorePanel.nodeId || n.type !== "asset")
                    return n;
                  const data = n.data as Record<string, unknown>;
                  const currentPreview = normalizeAssetAdjustments(
                    data.assetAdjustmentPreview
                  );
                  if (areAssetAdjustmentsEqual(currentPreview, adjustments))
                    return n;
                  return {
                    ...n,
                    data: { ...data, assetAdjustmentPreview: adjustments },
                  };
                })
              );
            }}
          ...
          >
            <div
              style={{
                ...
                borderRadius: "50%",
                background: isDark
                  ? "oklch(0.58 0.20 290 / 0.25)"
                  : "oklch(0.58 0.20 290 / 0.15)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
              }}
          ...
          {dragPos && (
            <div
              ...
              style={{
                ...
                borderRadius: "50%",
                background: isDark
                  ? "oklch(0.72 0.20 290 / 0.30)"
                  : "oklch(0.55 0.18 290 / 0.25)",
      ...
      <ReactFlow
        ...
        onEdgesChange={
          ENABLE_NODE_CONNECTIONS ? handleEdgesChangeWithHistory : undefined
        }
      ...
      <CropEditor
        ...
        onConfirm={(croppedDataUrl, size) => {
          ...
          setNodes(nds =>
            nds.map(n => {
              if (n.id !== cropEditorState.nodeId || n.type !== "asset")
                return n;
              const data = n.data as Record<string, unknown>;
              const currentSize = getCanvasNodeSize(n);
              const nextW = Math.max(
                ...
                Math.round(
                  currentSize.width *
                    (size.width / Math.max(1, size.naturalWidth))
                )
              );
              const nextH = Math.max(
                ...
                Math.round(
                  currentSize.height *
                    (size.height / Math.max(1, size.naturalHeight))
                )
              );
              return {
                ...n,
          ...
          toast("已完成裁切", {
            description: `${size.width} × ${size.height}px`,
          });
        }}
      />
      ...
      <RotateEditor
        ...
        onConfirm={(rotatedDataUrl, size) => {
          ...
          setNodes(nds =>
            nds.map(n => {
              if (n.id !== rotateEditorState.nodeId || n.type !== "asset")
                return n;
              const data = n.data as Record<string, unknown>;
              const currentSize = getCanvasNodeSize(n);
              const nextW = Math.max(
      ...
      <CanvasAssistantPanel
        ...
        onRemoveReference={id =>
          setReferencedAssets(prev => prev.filter(r => r.id !== id))
        }
        onRemoveAnnotationReference={id =>
      ...
      {renamingGroupId && (
        ...
        >
          <div
            ...
            style={{
              background: isDark
                ? "oklch(0.15 0.018 270)"
                : "oklch(0.995 0.002 80)",
            ...
            <input
              ...
              onKeyDown={e => {
                if (e.key === "Enter" && renameValue.trim()) {
                  setGroupNames(prev => ({
                    ...prev,
                    [renamingGroupId]: renameValue.trim(),
                  }));
      ...
      <ZoomControlBar
        ...
        onLockedChange={setIsCanvasLocked}
      />
      ...
      {nodeCtxMenu && (
        <NodeContextMenu
          ...
          onAction={(action, nodeId) => {
            if (
              nodeCtxMenu.nodeType === "group-container" ||
              nodeCtxMenu.nodeType === "group-container-inside"
            ) {
      ...
      {shapeCtxMenu &&
        (() => {
          ...
          const nw =
            (shapeNode.style?.width as number) ||
            ((shapeNode.data as Record<string, unknown>).width as number) ||
            100;
          const nh =
            (shapeNode.style?.height as number) ||
            ((shapeNode.data as Record<string, unknown>).height as number) ||
            100;
          ...
          const applyShapeParams = () => {
            const newOpacity = Math.max(
              0,
              Math.min(1, parseFloat(shapeCtxMenu.menuOpacity) / 100 || 1)
            );
            const newStrokeW = parseFloat(shapeCtxMenu.menuStrokeW) || 0;
            setNodes(nds =>
              nds.map(n =>
                n.id === shapeCtxMenu.nodeId
                  ? {
                      ...n,
                      data: {
                        ...n.data,
                        fill: shapeCtxMenu.menuFill,
                        stroke: shapeCtxMenu.menuStroke,
                        strokeWidth: newStrokeW,
                        opacity: newOpacity,
                      },
                    }
                  : n
          ...
          return (
            <div
              ...
              onMouseDown={e => e.stopPropagation()}
            >
              <p style={{ fontWeight: 700, marginBottom: 10, fontSize: 13 }}>
                图形参数
              </p>
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <input
                    type="color"
                    value={
                      shapeCtxMenu.menuFill.startsWith("#")
                        ? shapeCtxMenu.menuFill
                        : "#6366f1"
                    }
                    onChange={e =>
                      setShapeCtxMenu(s =>
                        s ? { ...s, menuFill: e.target.value } : s
                      )
                    }
                    style={{
                      width: 28,
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <input
                    type="color"
                    value={
                      shapeCtxMenu.menuStroke.startsWith("#")
                        ? shapeCtxMenu.menuStroke
                        : "#000000"
                    }
                    onChange={e =>
                      setShapeCtxMenu(s =>
                        s ? { ...s, menuStroke: e.target.value } : s
                      )
                    }
                    style={{
                      width: 28,
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    type="number"
                    min={0}
                    value={shapeCtxMenu.menuStrokeW}
                    onChange={e =>
                      setShapeCtxMenu(s =>
                        s ? { ...s, menuStrokeW: e.target.value } : s
                      )
                    }
                    style={{
                      width: "100%",
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    min={0}
                    max={100}
                    value={shapeCtxMenu.menuOpacity}
                    onChange={e =>
                      setShapeCtxMenu(s =>
                        s ? { ...s, menuOpacity: e.target.value } : s
                      )
                    }
                    style={{
                      width: "100%",
        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{
                borderBottom: `1px solid ${isDark ? "oklch(1 0 0 / 8%)" : "oklch(0 0 0 / 8%)"}`,
              }}
            >
              <div className="flex items-center gap-2.5">
                <div
                  className="w-7 h-7 rounded-[var(--radius-md-design)] flex items-center justify-center"
                ...
                >
                  {(() => {
                    const dlNodes = (
                      window as unknown as Record<string, unknown>
                    ).__artx_download_nodes__ as Node[] | undefined;
                  const singleDl = (
                    window as unknown as Record<string, unknown>
                  ).__artx_single_download__ as
                    | { title: string; src: string }
                    | undefined;
                    const frameDl = (
                      window as unknown as Record<string, unknown>
                    ).__artx_frame_download__ as Node | undefined;
            ...
            <div className="px-5 py-4">
              ...
              <div className="flex gap-2">
                {(
                  ((window as unknown as Record<string, unknown>)
                    .__artx_frame_download__
                    ? ["png", "jpg", "psd"]
                    : ["png", "jpg"]) as CanvasDownloadFormat[]
                ).map(fmt => (
                  <button
                    key={fmt}
            ...
            <div className="flex gap-2 px-5 pb-5">
              ...
              <button
                onClick={async () => {
                  setDownloadDialogOpen(false);
                  const dlNodes = (window as unknown as Record<string, unknown>)
                    .__artx_download_nodes__ as Node[] | undefined;
                  const singleDl = (
                    window as unknown as Record<string, unknown>
                  ).__artx_single_download__ as
                    | { title: string; src: string }
                    | undefined;
                  const frameDl = (window as unknown as Record<string, unknown>)
                    .__artx_frame_download__ as Node | undefined;

                  if (frameDl) {
                    const frameData = frameDl.data as Record<string, unknown>;
                    const frameTitle =
                      (frameData.title as string) ||
                    ...
                    if (!blob) {
                      toast("导出失败", {
                        description: "当前画板暂时无法导出，请稍后重试",
                      });
                    } else {
                      saveAs(blob, `${safeName}.${downloadFormat}`);
                      toast("导出完成", {
                        description: `已导出画板 ${safeName}.${downloadFormat}`,
                      });
                    }
                    (
                      window as unknown as Record<string, unknown>
                    ...
                    const blob = await imageSrcToFormatBlob(
                      singleDl.src,
                      downloadFormat === "psd" ? "png" : downloadFormat
                    );
                    if (!blob) {
                      toast("下载失败", {
                        description: "当前图片暂时无法保存，请稍后重试",
                      });
                    } else {
                      saveAs(
                        blob,
                        `${safeName}.${downloadFormat === "psd" ? "png" : downloadFormat}`
                      );
                    }
                    (
                      window as unknown as Record<string, unknown>
      ...
      {drawingRect &&
        (() => {
          ...
          // 根据当前工具模式渲染对应几何形轮廓
          const shapePreviewType = activeToolMode.startsWith("shape-draw:")
            ? activeToolMode.replace("shape-draw:", "")
            : null;
          const buildPreviewPath = (type: string, w: number, h: number) => {
            ...
            if (type === "star") {
              ...
              for (let i = 0; i < 10; i++) {
                ...
                const r = i % 2 === 0 ? Math.min(w, h) / 2 : Math.min(w, h) / 4;
                pts.push(
                  `${i === 0 ? "M" : "L"} ${w / 2 + r * Math.cos(angle)} ${h / 2 + r * Math.sin(angle)}`
                );
              }
              return pts.join(" ") + " Z";
            }
              ...
              {shapePreviewType ? (
                ...
                >
                  <path
                    ...
                    fill={
                      shapePreviewType === "line" ||
                      shapePreviewType === "arrow"
                        ? "none"
      ...
      {activeToolMode === "pen" &&
        ...
        (() => {
          ...
          // 将 flow 坐标转换为屏幕坐标
          const lastScreenX =
            (penNode.position.x + lastAnchor.x) * viewport.zoom + viewport.x;
          const lastScreenY =
            (penNode.position.y + lastAnchor.y) * viewport.zoom + viewport.y;
          return (
            <svg
              className="absolute pointer-events-none"
              style={{
      ...
      {pendingRect &&
        (() => {
          ...
          let popTop = ry;
          if (popTop + popMaxH > containerH)
            popTop = Math.max(8, containerH - popMaxH - 8);
                ...
                >
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <input
                      ...
                      onKeyDown={e => {
                        if (e.key === "Enter") handleCreateCanvasConfirm();
                        if (e.key === "Escape") handleCreateCanvasCancel();
                      }}
                  ...
                  {(() => {
                    ...
                    const selBg = isDark
                      ? "rgba(255,255,255,0.10)"
                      : "rgba(0,0,0,0.07)";
                    const hoverItemBg = isDark
                      ? "rgba(255,255,255,0.06)"
                      : "rgba(0,0,0,0.04)";
                    const isSelected = (pw: number, ph: number) =>
                      canvasInputW === String(pw) &&
                      canvasInputH === String(ph);
                        ...
                        {presetOpen &&
                          presets.map((p, i) => (
                            <button
                              key={p.label}
                              onClick={() => {
                                setCanvasInputW(String(p.w));
                                setCanvasInputH(String(p.h));
                                setPresetOpen(false);
                              }}
                              style={{
                                display: "flex",
                              ...
                              onMouseEnter={e => {
                                if (!isSelected(p.w, p.h))
                                  (
                                    e.currentTarget as HTMLElement
                                  ).style.background = hoverItemBg;
                              }}
                              onMouseLeave={e => {
                                (
                                  e.currentTarget as HTMLElement
                                ).style.background = isSelected(p.w, p.h)
                                  ? selBg
                                  : "transparent";
                              }}
                            >
                  ...
                  {(() => {
                    ...
                    const groupedPlatforms = Array.from(
                      new Set(
                        SOCIAL_MEDIA_SIZE_PRESETS.map(preset => preset.platform)
                      )
                    );
                    return (
                      <div
                        ...
                        >
                          ...
                          >
                            ...
                            {selectedSocialPreset && (
                              ...
                              >
                                {selectedSocialPreset.platform} ·{" "}
                                {selectedSocialPreset.title}
                              </span>
                            )}
                          </span>
                          <svg
                          ...
                          >
                            {groupedPlatforms.map(platform => (
                              <div key={platform}>
                                ...
                                {SOCIAL_MEDIA_SIZE_PRESETS.filter(
                                  preset => preset.platform === platform
                                ).map(preset => {
                                  const active =
                                    preset.id === canvasSocialPresetId;
                                  return (
                                    <button
                                      key={preset.id}
                                      onClick={() => {
                                      ...
                                      style={{
                                        ...
                                        background: active
                                          ? isDark
                                            ? "rgba(147,108,255,0.16)"
                                            : "rgba(147,108,255,0.10)"
                                          : "transparent",
                                        border: "none",
                  ...
                  {(() => {
                    ...
                    const [ch, cs, cv] = hexToHsv(
                      canvasBgColor.startsWith("#") &&
                    ...
                    const handleHueDrag = (
                      ...
                      setCanvasBgColor(
                        hsvToHex(
                          h,
                          colorHsvRef.current[1],
                          colorHsvRef.current[2]
                        )
                      );
                    };
                              ...
                              onChange={e => {
                                const v =
                                  ...
                                  e.target.value
                                    .replace(/[^0-9a-fA-F]/g, "")
                                    .slice(0, 6);
                                if (v.length === 7) setCanvasBgColor(v);
                              }}
                              style={{
                                flex: 1,
                                background: "transparent",
      ...
      {globalAnnotations.length > 0 && (
        <GlobalAnnotationLayer
          ...
          referencedAnnotationIds={
            new Set(annotationReferences.map(reference => reference.id))
          }
        />
      )}
    </div>
