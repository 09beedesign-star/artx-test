...
describe("需求 2：撤销按钮与快照失效边界", () => {
  ...
  it("按钮点击必须派发唯一的撤销事件，且不能把点击传给画布", () => {
    /*
     * ⚠️ 锚点变更史：单个「撤销」按钮 → 2026-09-23 变「撤销 + 确认」按钮组
     *    → 2026-09-24 用户要求移除确认，又回到单个「撤销」按钮。
     * ⚠️⚠️ 教训：起始锚点不要用带日期/带按钮清单的注释标题 —— 每次增删按钮
     *      都会把它改掉，测试会因为"注释变了"而假性变红。这里改用**条件渲染
     *      表达式本身**做锚点，它只在渲染条件真的变了时才会动。
     */
