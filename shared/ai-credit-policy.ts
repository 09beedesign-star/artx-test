...
export const AI_CREDIT_POLICIES: AiBillingPolicy[] = [
  ...
  },
  {
     ...
     *
     * ✅ 2026-09-13 已完成毛利核算，结论是 **200 不需要上调**，别再被「+69%」吓到：
     *   充值最优惠档是「大额补充」170 积分/元，即 200 积分 ≈ 1.176 元，
     *   扣掉 0.199 元成本后毛利 0.977 元，**毛利率 83.1%**（最坏档；
     *   轻量档 130 积分/元时毛利率 87.1%）。
     *   涨价前后的差别只是毛利率从 90.0% 降到 83.1%，仍远高于健康线。
     *
     * 📌 反过来说，若哪天想把毛利率主动压到 50%/60%/70% 让利用户，
     *    对应的 baseCredits 约为 70 / 80 / 110（按最坏档 170 积分/元换算）。
     *
     * ⚠️ 改这个数 = 改钱，属产品决策。任何调整都必须先把影响换算成积分/人民币
     *    报给用户确认，不要因为「上游涨价了」就自动跟涨。
     */
