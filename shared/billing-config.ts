 ...
 * 注册初始额度 —— **唯一真相来源**。
 *
 * 【2026-09-18 新增】解决的问题：新用户注册后余额为 0，
 * 什么都试不了就得先掏钱，转化漏斗在第一步就断了。
 *
 * ⚠️⚠️⚠️ 这是对「注册链路零成本」红线的**有意突破**，
 * 必须与注册限频**同时存在**，否则等同于一台永久提款机
 * （见 server/invite-rewards.ts 顶部说明、以及本文件下方邀请奖励的同款警告）。
 * 📌 任何人想调大 credits 或关掉 SIGNUP_IP_RATE_LIMIT，必须先回答：
 *    「一个脚本一晚上能薅走多少钱？」
 *
 * 【为什么是 350】
 * 默认档 medium 1K 是 70 积分/张，350 = 恰好 5 张。
 * 5 张的依据：AI 出图首张翻车是常态，3 张不够试错（体验变负分），
 * 10 张够完成一件真实小活（拿完即走，且薅号收益翻倍）。
 * 5 张「够试错、不够干活」—— 能体会到价值，但做不完正事，
 * 这个断点才是推动付费的地方。
 * 现金成本 5 × 0.078 元 ≈ ¥0.39/人；占 Lite 月卡 8000 积分的 4.4%，
 * 不会稀释套餐价值感。
 *
 * 【为什么有效期是 3 天而不是 2 天】
 * 2 天会误伤真实用户：周五晚注册、周末没空碰，周一想起来额度已没 ——
 * 而这批人恰恰是有真实需求的高价值用户。
 * 3 天能完整覆盖一个周末，对防刷的削弱几乎为零：
 * 刷号的人当场就把积分用光，他们不在乎是 2 天还是 3 天。
 * 📌 判据：2 天挡不住坏人，只挡住了好人。
 */
export const SIGNUP_INITIAL_CREDITS = {
  /** 总开关。关掉后新注册用户余额为 0（回到改造前的状态）。 */
  enabled: true,
  /** 注册即到账的初始额度 */
  credits: 350,
  /** 有效期（天）。刻意短于默认赠送的 30 天，制造使用紧迫感。 */
  expiryDays: 3,
  /**
   * 活动截止日期（ISO，含当天）。超过此日期注册的用户不再发放。
   * ⚠️ 设为 undefined = 永久发放，上线前务必确认这是你要的。
   */
  activeUntil: "2026-10-18" as string | undefined,
  /**
   * 配置版本号 —— **改这个文件里上面任何一个值，都必须把它 +1**。
   *
   * 【为什么需要它】本项目踩过两次同一个坑（plans、aiBillingPolicies）：
   * 配置被快照进生产库后，代码里的新值永远生效不了，且全程零报错。
   * 事故复盘见 server/admin-store.ts:2602 那段长注释
   * （扩图代码写 200 积分、实际只扣 16，7 项能力持续亏钱）。
   *
   * 当时的止血手段是「无条件用代码值」，代价是后台面板彻底失效。
   * 这里改用版本号方案，两者兼得：
   *   库值版本 >= 代码版本 → 用库值（后台改的生效）
   *   库值版本 <  代码版本 → 丢弃库值用代码值（代码改的能顶掉旧快照）
   *
   * 📌 判据：只要你改了上面的默认值又忘了 +1，你的改动就不会生效。
   *    signup-initial-credits.test.ts 里有一条测试专门盯着这件事。
   */
  configVersion: 1,
} as const;

/**
 * 注册初始额度的**安全上限** —— 后台可编辑带来的新风险由它兜底。
 *
 * ⚠️ 为什么必须有：注册链路零成本（无手机号/无验证码），
 * 初始额度本质是「谁来注册就给谁发钱」。一旦后台能改，
 * 误操作多打一个 0（350 → 3500）就等于把提款机开到十倍，
 * 且这类错误在审计日志里长得和正常操作一模一样。
 *
 * 上限依据：
 *   credits 1000 ≈ 默认档 14 张图 ≈ ¥1.1/人。按网段每日 30 个号算，
 *   单网段每日最坏损失 ¥33 —— 仍在可承受范围，再高就该先补注册门槛。
 *   expiryDays 30 对齐通用赠送积分有效期，超过就失去「紧迫感」的意义。
 */
export const SIGNUP_INITIAL_CREDITS_LIMITS = {
  maxCredits: 1000,
  maxExpiryDays: 30,
} as const;

/**
 * 注册 IP 限频 —— 初始额度的**配套风控**，两者必须同进同退。
 *
 * ⚠️⚠️ 为什么必须带 24 小时时间窗，而不是「同 IP 永久只能注册 3 次」：
 * 公司、学校、咖啡馆、手机基站 NAT 都是几十上百人共用一个出口 IP。
 * 永久限 3 次会把整栋楼的真实用户全部挡在门外，
 * 而且用户只会看到一句「注册失败」，客服根本查不出原因 ——
 * 这是三条风控里**唯一可能真正伤到生意**的一条。
 *
 * ⚠️⚠️⚠️ 为什么是「精确 IP + 网段」两层，而不是只按网段限 3 次：
 * 一个 /24 网段有 254 个地址，很可能就是一整栋写字楼或一所学校。
 * 只按网段限 3 次，等于一栋楼一天只能有 3 个人注册 —— 误伤面积大到荒谬。
 * 📌 这个缺陷是被 invite-refund-rate-ban.test.ts 抓出来的：
 *    它用 10.1.0.1~10.1.0.N 模拟多个被邀请人（真实场景就是同事互邀），
 *    在只有网段限制时第 4 个人就被 429 拦死。
 *
 * 分层依据：**同一个精确 IP 反复注册**才是刷号的强信号（脚本通常不换 IP）；
 * 而同网段不同 IP 更像是「同一栋楼的不同人」。所以精确 IP 收紧、网段放宽。
 *
 * 成本上限：网段每天 30 个号 × 350 积分 ≈ ¥11.7/天/网段，可接受。
 */
export const SIGNUP_IP_RATE_LIMIT = {
  /** 同一**精确 IP** 在时间窗内允许的注册次数 */
  maxPerWindow: 3,
  /**
   * 同一 IPv4 /24 网段（IPv6 为 /64）在时间窗内允许的注册次数。
   * 必须显著大于 maxPerWindow，否则共享出口的正常用户会被误伤。
   */
  maxPerSubnetWindow: 30,
  /** 滚动时间窗（小时） */
  windowHours: 24,
} as const;
