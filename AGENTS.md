...
推它 = 推正式站，**没有隔离**。**听到这个说法要先向用户确认意图**，不要默认按「正式环境」执行。

⚠️ 证书 SAN 实测只有 `artxsd.com` / `backstage.artxsd.com` / `www.artxsd.com`，
**`gray.artxsd.com` 既无 DNS 解析也不在 SAN 内**（旧笔记说「已在 SAN 内」是错的）。
要建独立测试站需要：新 DNS 记录 + 扩 SAN + 独立进程端口 + 独立数据库，四样缺一不可。

## 开发阶段怎么让用户看不到（ericbi 2026-09-24 拍板）

**分支不是环境。代码在哪个分支跟用户能不能看到无关，只跟「有没有跑部署」有关。**

- 新功能开发期：正常提交、推 `main`。因为 `push:` 触发器已移除，**推多少次线上都不变**。
- 自己要看效果 → **走本机预览**：`npm run dev` → `http://localhost:3000`
  （已带 `--host`，同一 WiFi 手机可用 `http://<本机内网IP>:3000`）。
  ⚠️ 该 dev 进程常年挂着不退，起之前先 `lsof -nP -iTCP:3000 -sTCP:LISTEN`，
  是本项目的 vite 就直接复用，不要 kill 了重起。
- 功能验完、用户说「推送到正式环境」→ 才手动触发部署。
- **不要为了「看效果」而部署到线上**，那等于直接发给真实用户。
